# Architecture Decision Records

ADRs are immutable decision records. Superseded decisions remain in the repository and link to their replacements.

## Registry

- [ADR-0001 — Offline-First Architecture](ADR-0001-offline-first.md)
- [ADR-0002 — Database-Independent Domain](ADR-0002-database-independent-domain.md)
- [ADR-0003 — Modular Monolith](ADR-0003-modular-monolith.md)
- [ADR-0004 — SQLite Behind Infrastructure Contracts](ADR-0004-sqlite-infrastructure.md)
- [ADR-0005 — Repository and Unit of Work](ADR-0005-repository-unit-of-work.md)
- [ADR-0006 — Application Service Boundary](ADR-0006-application-services.md)
- [ADR-0007 — Immutable Audit Trail](ADR-0007-immutable-audit-trail.md)
- [ADR-0008 — Approval Workflow with Optimistic Concurrency](ADR-0008-approval-optimistic-concurrency.md)
- [ADR-0009 — Shared Platform Infrastructure Before Accounting Core](ADR-0009-platform-infrastructure-first.md)
- [ADR-0010 — Chart of Accounts Model](ADR-0010-chart-of-accounts-model.md)
- [ADR-0011 — Independent Accounting Dimensions](ADR-0011-independent-accounting-dimensions.md)

Use the [ADR template](../templates/adr-template.md) for new decisions.
