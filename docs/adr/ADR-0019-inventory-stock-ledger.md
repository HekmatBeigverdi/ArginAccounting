...
1. Confirmed stock effects are represented as immutable `InventoryStockMovementSnapshot` facts with durable `movementId`, source document/line IDs, business date, stable `businessOrder`, UTC recording evidence, signed base-unit quantity and a durable StockKey.
...
5. Canonical chronology is deterministic: `businessDate`, then positive stable `businessOrder`, then durable `documentId`, `lineId`, and `movementId` tie breakers. `recordedAt` is evidence and does not decide stock chronology.
6. `businessOrder` is assigned by the authoritative confirmation/persistence boundary in later Steps 10–13 and persisted as part of the movement fact; random UUID ordering, SQLite rowid and local arrival order are not acceptable substitutes.
7. Default policy rejects any ledger in which a StockKey becomes negative at any point in canonical history. Therefore a newly inserted backdated movement must revalidate historical running balances, not only the current ending balance.
8. A policy flag may explicitly permit negative stock for deployments that choose that behavior; permissive mode does not change canonical ordering or movement immutability.
9. Duplicate `movementId` is invalid. A document line cannot create the same StockKey fact twice; the same line may later create different StockKey facts where the owning workflow requires it (for example, transfer source/destination in Step 8).
10. Reversal does not delete or edit prior movements. Step 8 will emit compensating movement facts linked to the reversal lifecycle defined in Step 5.
11. Persistence, transaction locking, idempotency storage and atomic confirmation remain Steps 10–13. This ADR defines Domain truth, not a SQLite implementation.
...
- Same-day ordering is stable across SQLite, PostgreSQL and Bridge replicas because `businessOrder` is persisted, with durable identities only as tie breakers.
...
- Ordering by database sequence/rowid, UUID lexical order, `recordedAt` or local insertion order: rejected because these can differ from business chronology or between offline replicas.
