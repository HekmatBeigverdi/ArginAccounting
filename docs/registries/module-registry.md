...
| Purchase Posting | `@argin/purchase-posting`, `@argin/purchase-posting-tauri` | Permissions, Audit and Traceability implemented through Phase 23 Step 23 | 23 | [Purchase Posting Domain Model](../architecture/purchase-posting-domain-model.md) |
...
- Domain/Application: `packages/purchase-posting` now owns secured Posting/Reversal/Rule/Trace application contracts through Step 23; `packages/purchase-posting-tauri` remains the SQLite adapter boundary.
...
- Testing: Step 23 adds persisted-scope authorization, trace-integrity, Audit identity and Security permission-catalog coverage.
