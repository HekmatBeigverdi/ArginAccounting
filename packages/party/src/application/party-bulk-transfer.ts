...
export class PartyBulkTransferService {
  ...
  async import(rows: readonly PartyTabularRow[], mapping: PartyImportColumnMap, context: PartyImportContext, options: { readonly atomic: boolean }): Promise<PartyImportResult> {
    ...
    const validRows = normalized.filter((entry) => entry.preview.valid);

    if (options.atomic) {
      await this.unitOfWork.run(async ({ parties }) => {
        for (const entry of validRows) {
          const party = materialize(entry, context, this.ids.nextId());
          if (await parties.findByCode(context.companyId, party.code)) throw new Error(`party.code.conflict:${party.code}`);
      ...
      for (const entry of validRows) {
        try {
          await this.unitOfWork.run(async ({ parties }) => {
            const party = materialize(entry, context, this.ids.nextId());
            if (await parties.findByCode(context.companyId, party.code)) throw new Error("party.code.conflict");
            await parties.add(party);
          });
          importedCount += 1;
        } catch (error) {
          runtimeFailures.push(Object.freeze({ ...entry.preview, valid: false, issues: Object.freeze([...entry.preview.issues, Object.freeze({ code: "party.import.writeFailed", message: error instanceof Error ? error.message : "Import write failed." })]) }));
        }
      }
    }
    ...
    return Object.freeze({ importedCount, failedCount: runtimeFailures.length, atomic: options.atomic, failures: Object.freeze(runtimeFailures) });
  }
...
function mapRow(row: PartyTabularRow, mapping: PartyImportColumnMap): PartyImportCreateInput {
  ...
  const contacts = [contact("phone", readMapped(row, mapping.phone)), contact("mobile", readMapped(row, mapping.mobile)), contact("email", readMapped(row, mapping.email))].filter((value): value is NonNullable<typeof value> => value !== null);
  ...
  if (classification === "natural-person") {
    return { classification, code, firstName: readMapped(row, mapping.firstName), lastName: readMapped(row, mapping.lastName), roles, identity: { nationalCode: nullable(readMapped(row, mapping.nationalCode)), economicNumber: nullable(readMapped(row, mapping.economicNumber)), taxFileNumber: nullable(readMapped(row, mapping.taxFileNumber)) }, contacts, addresses };
  }
  return { classification, code, legalName: readMapped(row, mapping.legalName), tradeName: nullable(readMapped(row, mapping.tradeName)), roles, identity: { nationalId: nullable(readMapped(row, mapping.nationalId)), registrationNumber: nullable(readMapped(row, mapping.registrationNumber)), economicNumber: nullable(readMapped(row, mapping.economicNumber)), legacyEconomicCode: nullable(readMapped(row, mapping.legacyEconomicCode)), taxFileNumber: nullable(readMapped(row, mapping.taxFileNumber)) }, contacts, addresses };
}
...
function buildProbe(party: Party): PartyDuplicateProbe { return Object.freeze({ companyId: party.companyId, excludePartyId: null, code: party.code, classification: party.classification, displayName: party.displayName, nationalCode: party.classification === "natural-person" ? party.identity.nationalCode : null, nationalId: party.classification === "legal-entity" ? party.identity.nationalId : null, economicNumber: party.identity.economicNumber }); }
function normalizeClassification(value: string): "natural-person" | "legal-entity" { const normalized = value.trim().toLowerCase(); if (["natural-person", "natural", "individual", "حقیقی"].includes(normalized)) return "natural-person"; if (["legal-entity", "legal", "company", "حقوقی"].includes(normalized)) return "legal-entity"; throw new Error("Party classification must identify a natural person or legal entity."); }
function parseRoles(value: string): readonly PartyRole[] { if (!value) return Object.freeze([]); const roles: PartyRole[] = []; for (const token of value.split(/[،,;|]/).map((item) => item.trim().toLowerCase()).filter(Boolean)) { if (["customer", "مشتری"].includes(token)) roles.push("customer"); else if (["supplier", "vendor", "تامین کننده", "تأمین‌کننده", "تأمین کننده"].includes(token)) roles.push("supplier"); else throw new Error(`Unsupported Party role: ${token}`); } return Object.freeze([...new Set(roles)]); }
function contact(type: "phone" | "mobile" | "email", value: string) { return value ? { id: `import-${type}`, type, value, purpose: "general" as const, isPrimary: true } : null; }
function summarize(rows: readonly PartyImportPreviewRow[]): PartyImportPreview { const validRows = rows.filter((row) => row.valid).length; return Object.freeze({ totalRows: rows.length, validRows, invalidRows: rows.length - validRows, rows: Object.freeze([...rows]) }); }
function findBatchDuplicateRows(rows: readonly PreparedRow[]): ReadonlySet<number> { const keys = new Map<string, number[]>(); for (const row of rows.filter((entry) => entry.preview.valid)) { const party = row.previewParty; const values = [`code:${party.code}`, party.classification === "natural-person" && party.identity.nationalCode ? `nationalCode:${party.identity.nationalCode}` : null, party.classification === "legal-entity" && party.identity.nationalId ? `nationalId:${party.identity.nationalId}` : null, party.identity.economicNumber ? `economic:${party.identity.economicNumber}` : null].filter((value): value is string => value !== null); for (const key of values) keys.set(key, [...(keys.get(key) ?? []), row.rowNumber]); } return new Set([...keys.values()].filter((items) => items.length > 1).flat()); }
...
function readMapped(row: PartyTabularRow, sourceColumn: string | undefined): string { return sourceColumn ? String(row[sourceColumn] ?? "").trim() : ""; }
function nullable(value: string): string | null { return value.length === 0 ? null : value; }
function authContext(context: PartyImportContext) { return Object.freeze({ actorId: context.actorId, companyId: context.companyId, correlationId: context.correlationId, requestId: context.requestId }); }
