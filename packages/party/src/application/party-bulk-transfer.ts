...
export const partyImportFields = [
  ...
  "nationalCode", "nationalId", "registrationNumber", "economicNumber", "taxFileNumber",
...
function mapRow(row: PartyTabularRow, mapping: PartyImportColumnMap): PartyImportCreateInput {
  ...
  if (classification === "natural-person") {
    return {
      classification, code, firstName: readMapped(row, mapping.firstName), lastName: readMapped(row, mapping.lastName), roles,
      identity: { nationalCode: nullable(readMapped(row, mapping.nationalCode)), economicNumber: nullable(readMapped(row, mapping.economicNumber)), taxFileNumber: nullable(readMapped(row, mapping.taxFileNumber)) },
      contacts, addresses
    };
  }
  return {
    classification, code, legalName: readMapped(row, mapping.legalName), tradeName: nullable(readMapped(row, mapping.tradeName)), roles,
    identity: { nationalId: nullable(readMapped(row, mapping.nationalId)), registrationNumber: nullable(readMapped(row, mapping.registrationNumber)), economicNumber: nullable(readMapped(row, mapping.economicNumber)), taxFileNumber: nullable(readMapped(row, mapping.taxFileNumber)) },
    contacts, addresses
  };
}
...
function findBatchDuplicateRows(rows: readonly PreparedRow[]): ReadonlySet<number> {
  ...
  for (const row of rows.filter((entry) => entry.preview.valid)) {
    ...
    const values = [`code:${party.code}`, party.classification === "natural-person" && party.identity.nationalCode ? `nationalCode:${party.identity.nationalCode}` : null, party.classification === "legal-entity" && party.identity.nationalId ? `nationalId:${party.identity.nationalId}` : null, party.identity.economicNumber ? `economic:${party.identity.economicNumber}` : null].filter((value): value is string => value !== null);
