...
export interface PartyMasterExportPage {
  readonly items: readonly PartyMasterExportRow[];
...
export interface PartyMasterExportReader {
  listPage(companyId: string, page: number, pageSize: number): Promise<PartyMasterExportPage>;
}

export interface PartyMasterExportSink {
  write(rows: readonly PartyMasterExportRow[]): Promise<void>;
}

export class PartyMasterExportService {
  constructor(
    private readonly reader: PartyMasterExportReader,
  ...
  async export(context: PartyImportContext, sink: PartyMasterExportSink, pageSize = 500): Promise<number> {
    if (!Number.isInteger(pageSize) || pageSize < 1 || pageSize > 2000) {
      throw new RangeError("Party export pageSize must be between 1 and 2000.");
    }
    ...
    await this.audit.record(Object.freeze({
      ...
      metadata: Object.freeze({ exportedCount: exported, pageSize, mode: "master-data" })
    }));
    return exported;
  }
}
