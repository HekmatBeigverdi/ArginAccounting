import type { ProductSelectorItemDto } from "./contracts/product-dto.ts";
...
export interface ProductSelectorOption extends ProductSelectorItemDto {
  /** Canonical foreign identity for downstream modules. */
...
function normalizeLimit(limit: number | undefined): number {
  ...
  if (!Number.isSafeInteger(value) || value < PRODUCT_QUERY_LIMITS.minSelectorLimit || value > PRODUCT_QUERY_LIMITS.maxSelectorLimit) {
    throw new ProductApplicationError(
      PRODUCT_APPLICATION_ERROR_CODES.invalidRequest,
...
> {
  switch (usage) {
    case "inventory": return { statuses: ["active"], kinds: ["product"], stockTracking: true };
    case "purchase": return { statuses: ["active"], purchasable: true };
    case "sales": return { statuses: ["active"], sellable: true };
    case "taxpayer": return { statuses: ["active"], requiresTaxpayerGoodsServiceId: true };
    case "manufacturing": return { statuses: ["active"], kinds: ["product"] };
    case "cost-accounting": return { statuses: ["active"], kinds: ["product"] };
    case "general": return { statuses: ["active"] };
  }
}

function intersect<T extends string>(required: readonly T[] | undefined, requested: readonly T[] | undefined): readonly T[] | undefined {
...
export function createProductSelectorQuery(request: ProductSelectorRequest): ProductSelectorQuery {
  const companyId = requiredText(request.companyId, "companyId");
  if (!productSelectorUsages.includes(request.usage)) {
    throw new ProductApplicationError(PRODUCT_APPLICATION_ERROR_CODES.invalidRequest, "Product selector usage is invalid.");
  }
  ...
  return {
    ...
    ...(request.categoryIds === undefined ? {} : { categoryIds: request.categoryIds.map((value) => requiredText(value, "categoryId")) }),
    ...
    ...(defaults.requiresTaxpayerGoodsServiceId === undefined ? {} : { requiresTaxpayerGoodsServiceId: defaults.requiresTaxpayerGoodsServiceId }),
