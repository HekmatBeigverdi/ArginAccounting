...
export class PartyTabularCodecError extends Error {
  constructor(
    readonly code: "party.import.fileTooLarge" | "party.import.unreadable" | "party.import.rowLimit" | "party.import.columnLimit" | "party.import.empty",
    message: string
  ) {
    super(message);
...
export function parsePartyXlsx(bytes: Uint8Array): PartyTabularData {
  if (bytes.byteLength > PARTY_TABULAR_LIMITS.maximumFileSizeBytes) {
    throw new PartyTabularCodecError("party.import.fileTooLarge", "Party Excel file exceeds the allowed size.");
  }
  let workbook: WorkBook;
  try {
    workbook = read(bytes, { type: "array", cellDates: false, cellFormula: false, dense: false });
  } catch {
    throw new PartyTabularCodecError("party.import.unreadable", "Party Excel file cannot be read.");
  }
  const firstSheetName = workbook.SheetNames[0];
  if (!firstSheetName) throw new PartyTabularCodecError("party.import.empty", "Party Excel file has no worksheet.");
  const sheet = workbook.Sheets[firstSheetName];
  if (!sheet) throw new PartyTabularCodecError("party.import.empty", "Party Excel worksheet is missing.");
  return parseWorksheet(sheet);
}

export function parsePartyCsv(text: string): PartyTabularData {
  if (new TextEncoder().encode(text).byteLength > PARTY_TABULAR_LIMITS.maximumFileSizeBytes) {
    throw new PartyTabularCodecError("party.import.fileTooLarge", "Party CSV file exceeds the allowed size.");
  }
  let workbook: WorkBook;
  try {
    workbook = read(text, { type: "string", raw: true });
  } catch {
    throw new PartyTabularCodecError("party.import.unreadable", "Party CSV file cannot be read.");
  }
...
function parseWorksheet(sheet: WorkSheet): PartyTabularData {
  ...
  if (headers.length > PARTY_TABULAR_LIMITS.maximumColumns) {
    throw new PartyTabularCodecError("party.import.columnLimit", "Party tabular file has too many columns.");
  }
  const dataRows = matrix.slice(1);
  if (dataRows.length > PARTY_TABULAR_LIMITS.maximumRows) {
    throw new PartyTabularCodecError("party.import.rowLimit", "Party tabular file has too many rows.");
  }
