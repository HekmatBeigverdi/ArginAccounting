...
import { createInventoryStockKey, rebuildInventoryStockLedger, serializeInventoryStockKey } from "../domain/inventory-stock.ts";
...
export interface InventoryApplicationIdentityFactory {
  movementId(input: { readonly documentId: string; readonly lineId: string; readonly role: "single" | "source" | "destination" | "reversal" }): string;
...
export interface InventoryCurrentMasterResolver {
  ...
  warehouse(companyId: string, reference: { readonly warehouseId: string; readonly zoneId?: string | null; readonly locationId?: string | null }): Promise<InventoryWarehouseResolution>;
}

export interface InventoryNumberingGateway {
  /** Called only when a lifecycle operation requires a display number and the document has none. */
...
async function persistOutcome(
  ...
  return Object.freeze({ documentId: document.documentId, status: document.status, version: document.version, replayed: false });
}
...
function mapDomainError(error: unknown): never {
  ...
  if (error.code === INVENTORY_DOMAIN_ERROR_CODES.duplicateMovementId || error.code === INVENTORY_DOMAIN_ERROR_CODES.duplicateMovementSource) {
    return fail(appCodes.duplicateMovement, error.field);
  }
  return fail(appCodes.invalidRequest, error.field);
}

async function loadDocument(context: InventoryUnitOfWorkContext, companyId: string, documentId: string): Promise<InventoryDocumentSnapshot> {
...
async function buildAuthoritativeLedger(context: InventoryUnitOfWorkContext, document: InventoryDocumentSnapshot) {
  const byMovementId = new Map<string, Awaited<ReturnType<InventoryUnitOfWorkContext["movements"]["listByStockKey"]>>[number]>();
  for (const line of document.lines) {
    if (!line.operation) continue;
    const references = [line.operation.warehouse, line.operation.destination].filter((item): item is NonNullable<typeof item> => item !== null);
    for (const warehouse of references) {
      const stockKey = createInventoryStockKey({ companyId: document.companyId, productId: line.productId, warehouse });
      for (const movement of await context.movements.listByStockKey(stockKey)) byMovementId.set(movement.movementId, movement);
    }
  }
  return rebuildInventoryStockLedger([...byMovementId.values()], { allowNegativeStock: true });
}

async function resolveCoreLines(deps: InventoryApplicationServiceDependencies, document: InventoryDocumentSnapshot): Promise<readonly InventoryLineConfirmationResolution[]> {
...
function coreMovementIdentities(deps: InventoryApplicationServiceDependencies, document: InventoryDocumentSnapshot): readonly InventoryLineMovementIdentity[] {
  return Object.freeze(document.lines.map(line => Object.freeze({
    lineId: line.lineId,
    movementId: deps.identities.movementId({ documentId: document.documentId, lineId: line.lineId, role: "single" }),
  })));
}

async function existingOpeningKeys(context: InventoryUnitOfWorkContext, document: InventoryDocumentSnapshot): Promise<readonly InventoryOpeningBalanceKey[]> {
  ...
  for (const line of document.lines) {
    ...
    const key: InventoryOpeningBalanceKey = Object.freeze({
      ...
      stockKey: createInventoryStockKey({ companyId: document.companyId, productId: line.productId, warehouse: line.operation.warehouse }),
    });
...
async function resolveTransferLines(deps: InventoryApplicationServiceDependencies, document: InventoryDocumentSnapshot): Promise<readonly InventoryTransferLineResolution[]> {
  return Promise.all(document.lines.map(async line => {
...
function transferMovementIdentities(deps: InventoryApplicationServiceDependencies, document: InventoryDocumentSnapshot): readonly InventoryTransferLineMovementIdentity[] {
  return Object.freeze(document.lines.map(line => Object.freeze({
    lineId: line.lineId,
    sourceMovementId: deps.identities.movementId({ documentId: document.documentId, lineId: line.lineId, role: "source" }),
    destinationMovementId: deps.identities.movementId({ documentId: document.documentId, lineId: line.lineId, role: "destination" }),
  })));
}
...
export class InventoryApplicationService {
  ...
  private async lifecycle(
    ...
    return this.deps.uow.execute(async context => {
      ...
      try {
        if (kind === "submit" && current.documentNumber === null) current = await this.deps.numbering.assign(current);
        const next = kind === "submit" ? submitInventoryDocument(current, command.action)
          : kind === "approve" ? approveInventoryDocument(current, command.action)
  ...
  submit(command: InventoryLifecycleCommand): Promise<InventoryApplicationMutationResult> { return this.lifecycle(command, "submit"); }
  approve(command: InventoryLifecycleCommand): Promise<InventoryApplicationMutationResult> { return this.lifecycle(command, "approve"); }
  cancel(command: InventoryLifecycleCommand): Promise<InventoryApplicationMutationResult> { return this.lifecycle(command, "cancel"); }
  ...
  async confirm(command: ConfirmInventoryDocumentCommand): Promise<InventoryApplicationMutationResult> {
    ...
    return this.deps.uow.execute(async context => {
      ...
      try {
        ...
        let movements;
        let balances;
        let openingKeys: readonly InventoryOpeningBalanceKey[] = [];
        if (document.documentType === "receipt" || document.documentType === "issue" || document.documentType === "opening") {
          const result = await confirmInventoryReceiptIssueOpening({
            ...
            ledger,
            openingKeys: await existingOpeningKeys(context, document),
            allowNegativeStock: command.allowNegativeStock,
          });
          confirmed = result.document; movements = result.movements; balances = result.ledger.balances;
          openingKeys = result.openingKeys.filter(key => key.companyId === document.companyId && document.scope && key.fiscalYearId === document.scope.fiscalYearId);
        ...
        if (document.documentType === "opening") {
          const previous = new Set((await existingOpeningKeys(context, document)).map(serializeInventoryOpeningBalanceKey));
          await context.openings.addBatch(openingKeys.filter(key => !previous.has(serializeInventoryOpeningBalanceKey(key))));
        }
        await context.documents.update(confirmed, command.expectedVersion);
        return persistOutcome(context, command, operation, "confirmation", confirmed, command.action.occurredAt);
      } catch (error) { return mapDomainError(error); }
    });
  }
  ...
  async reverse(command: ReverseInventoryCommand): Promise<InventoryApplicationMutationResult> {
    ...
    return this.deps.uow.execute(async context => {
      ...
      const ledger = await buildAuthoritativeLedger(context, document);
      const businessOrder = await context.businessOrders.next(document.companyId, document.businessDate);
      const identities: InventoryReversalMovementIdentity[] = originals.map(movement => ({
        originalMovementId: movement.movementId,
        reversalMovementId: this.deps.identities.movementId({ documentId: command.action.reversalDocumentId, lineId: movement.lineId, role: "reversal" }),
      }));
      try {
        ...
        return persistOutcome(context, command, operation, "reversal", result.document, command.action.occurredAt);
      } catch (error) { return mapDomainError(error); }
    });
  }
}
