...
const metadata = Object.freeze({
  ...
  templateCode: "excel-service",
  persianName: "الگوی خدماتی اکسل",
  englishName: "Excel service",
  activityType: "service" as const,
});

function parseErrorResult(): CodingTemplateWorkbookParseResult {
  return {
    success: false,
    metadata: null,
    content: null,
...
function successfulParseResult(): CodingTemplateWorkbookParseResult {
  return {
    success: true,
    metadata,
  ...
  const repositories = {
    ...
    codingTemplates: {
      ...
      findByCode: async (code: string) => {
        return (
          (templates as Array<{ code: string }>).find(
    ...
    codingTemplateImports: {
      create: async (value: CodingTemplateImportHistory) => {
        imports.push(value);
      },
      findByImportKey: async (key: string) => {
        return imports.find((item) => item.importKey === key) ?? null;
      },
    },
  ...
  const unitOfWork = {
    async run<T>(
      ...
      try {
        ...
        templates.length = before[0];
        versions.length = before[1];
        imports.length = before[2];
        throw error;
      }
    },
  };
...
test("previews a valid workbook without writing and returns a complete dry-run summary", async () => {
  ...
  const expectedAccountCount = IRAN_SERVICE_CODING_CATALOG.content.accounts.length;
  assert.equal(preview.summary.accountCount, expectedAccountCount);

  const { accountCount, dimensionTypeCount, dimensionMemberCount, accountDimensionPolicyCount } = preview.summary;
  const expectedTotalItemCount = accountCount + dimensionTypeCount + dimensionMemberCount + accountDimensionPolicyCount;
  assert.equal(preview.summary.totalItemCount, expectedTotalItemCount);
...
test("returns parser cell errors and writes nothing for an invalid workbook", async () => {
  ...
  await assert.rejects(
    () => importCodingTemplateWorkbook(f.command, f.dependencies),
    (error: unknown) =>
      error instanceof CodingTemplateWorkbookImportError &&
...
test("maps cross-sheet graph errors back to the originating row and cell field", async () => {
  ...
  const invalidPolicies = IRAN_SERVICE_CODING_CATALOG.content.accountDimensionPolicies.map(
    (item, index) =>
      index === 0
        ? { ...item, dimensionTypeLogicalKey: "missing" }
        : item,
  );

  const invalid = {
    ...IRAN_SERVICE_CODING_CATALOG.content,
    accountDimensionPolicies: invalidPolicies,
  };

  f.parser.parse = async () => ({
    success: true,
    metadata,
    content: invalid,
    issues: [],
  ...
  if (issue?.source === "graph") {
    assert.equal(issue.location?.sheet, "AccountDimensionPolicies");
    assert.equal(issue.location?.column, "dimensionTypeLogicalKey");
    assert.equal(issue.location?.address, "B2");
  } else {
    assert.fail("Expected a graph issue");
  }
});
...
test("imports template, version, provenance, and batch history in one transaction", async () => {
  ...
  const importRecord = f.imports[0];
  assert.equal(importRecord?.fileFingerprint, hash);
  assert.equal(importRecord?.contractVersion, "1.0");
  assert.equal(importRecord?.actorId, "admin");
  assert.equal(importRecord?.status, "published");

  assert.equal(f.events.length, 1);

  const event = f.events[0] as { eventType: string };
  assert.equal(event.eventType, "accounting.coding-template.imported");
});
...
test("returns the prior batch on a retry and prevents duplicates", async () => {
  ...
  const stateAfterFirstImport = f.state();
  ...
  assert.deepEqual(f.state(), stateAfterFirstImport);
  assert.equal(f.events.length, 1);
});
...
test("rejects unconfirmed, stale, unauthorized, and reused import identities before writes", async () => {
  ...
  await assert.rejects(
    () =>
      importCodingTemplateWorkbook(
        { ...f.command, expectedFileFingerprint: "b".repeat(64) },
        f.dependencies,
      ),
    (error: unknown) =>
      error instanceof CodingTemplateWorkbookImportError &&
  ...
  await assert.rejects(
    () => importCodingTemplateWorkbook(denied.command, denied.dependencies),
    (error: unknown) =>
      error instanceof CodingTemplateWorkbookImportError &&
      error.code === "permission_denied",
  );

  // Rejects reused import key
  await importCodingTemplateWorkbook(f.command, f.dependencies);
  f.dependencies.fingerprintProvider.sha256 = async () => "c".repeat(64);
  await assert.rejects(
    () =>
      importCodingTemplateWorkbook(
        { ...f.command, expectedFileFingerprint: "c".repeat(64) },
        f.dependencies,
      ),
    (error: unknown) =>
      error instanceof CodingTemplateWorkbookImportError &&
      error.code === "import_key_reused",
  );
});
