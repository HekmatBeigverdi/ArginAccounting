...
export async function handleSubmitJournalVoucherLifecycleCommand(
  ...
  dependencies: Pick<
    JournalVoucherLifecycleCommandDependencies,
...
export async function handleDecideJournalVoucherLifecycleApprovalCommand(
  ...
  await authorize(
    actionForDecision(command.decision),
    command.voucherId,
    command.context.actorId,
    dependencies.authorization,
  );
...
async function authorize(
  ...
  voucherId: string,
  actorId: string,
  dependencies: JournalVoucherLifecycleAuthorizationDependencies,
) {
  await assertJournalVoucherLifecycleAuthorized({
    action,
    voucherId,
    actorId,
    dependencies,
...
function actionForDecision(
  decision: DecideJournalVoucherLifecycleApprovalCommand["decision"],
): JournalVoucherLifecycleAuthorizedAction {
  switch (decision) {
    case "approve":
      return "approve";
