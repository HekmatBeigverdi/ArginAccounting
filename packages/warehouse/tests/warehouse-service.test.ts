...
const warehouses: WarehouseRepository = {
  async findById(companyId, warehouseId) {
    return states.get(warehouseKey(companyId, warehouseId)) ?? null;
  },
  async findByCode(companyId, code) {
    return [...states.values()].find(
      (state) => state.warehouse.companyId === companyId && state.warehouse.code === code,
    ) ?? null;
  },
  async findByExternalIdentifier(companyId, namespace, value) {
    return [...states.values()].find(
      (state) =>
        state.warehouse.companyId === companyId &&
        state.externalIdentifiers.some(
          (identifier) => identifier.namespace === namespace && identifier.value === value,
        ),
    ) ?? null;
  },
  async add(state) {
    states.set(warehouseKey(state.warehouse.companyId, state.warehouse.warehouseId), state);
  },
...
const zoneRepository: WarehouseZoneRepository = {
  async findById(companyId, zoneId) {
    const zone = zones.get(zoneId) ?? null;
    return zone?.companyId === companyId ? zone : null;
  },
  async listByWarehouse(companyId, warehouseId) {
    return [...zones.values()].filter(
      (zone) => zone.companyId === companyId && zone.warehouseId === warehouseId,
    );
  },
  async add(zone) {
...
const locationRepository: WarehouseLocationRepository = {
  async findById(companyId, locationId) {
    const location = locations.get(locationId) ?? null;
    return location?.companyId === companyId ? location : null;
  },
  async listByWarehouse(companyId, warehouseId) {
    return [...locations.values()].filter(
      (location) =>
        location.companyId === companyId && location.warehouseId === warehouseId,
    );
  },
  async listByZone(companyId, warehouseId, zoneId) {
    return [...locations.values()].filter(
      (location) =>
...
const reader: WarehouseReader = {
  ...
  async list(query) {
    return { items: [], page: query.page.page, pageSize: query.page.pageSize, totalCount: 0 };
  },
  async select() { return []; },
...
const idempotency: WarehouseIdempotencyExecutor = {
  async run(scope, requestId, work) {
    ...
    if (idempotentResults.has(key)) {
      return idempotentResults.get(key) as Awaited<ReturnType<typeof work>>;
    }
...
const service = new WarehouseService({
  ...
  branches: {
    async findById(companyId, branchId) {
      if (branchId === "inactive") {
        return { branchId, companyId, status: "inactive" };
      }
...
test("creates a versioned warehouse and replays the same request idempotently", async () => {
  ...
  const command = {
    ...
    organizationalScope: { mode: "company" as const },
    externalIdentifiers: [{ namespace: "erp", value: "A-1" }],
...
test("rejects duplicate company-scoped code and stale expected version", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      error.code === WAREHOUSE_APPLICATION_ERROR_CODES.duplicateIdentifier,
  );
  ...
  await assert.rejects(
    () => service.update({
      ...
      title: "A2",
      expectedVersion: 2,
      occurredAt: "2026-09-04T12:02:00Z",
    }),
    (error: unknown) =>
      error instanceof WarehouseApplicationError &&
      error.code === WAREHOUSE_APPLICATION_ERROR_CODES.concurrencyConflict,
  );
});
...
test("enforces active branch assignment and versions real lifecycle changes", async () => {
  ...
  await assert.rejects(
    () => service.create({
      requestId: "req-inactive-branch",
      warehouseId: "warehouse-1",
      companyId: "company-1",
      code: "WH-1",
      title: "انبار",
      kind: "general",
      organizationalScope: { mode: "branch", branchId: "inactive" },
      occurredAt: "2026-09-04T12:00:00Z",
    }),
  );

  const created = await service.create({
    requestId: "req-active-branch",
    warehouseId: "warehouse-1",
    companyId: "company-1",
    code: "WH-1",
    title: "انبار",
  ...
  const inactive = await service.changeStatus({
    ...
    occurredAt: "2026-09-04T12:10:00Z",
  });
...
test("validates warehouse/zone/parent boundaries for physical master data", async () => {
  ...
  const rack = await service.createLocation({
    requestId: "req-rack",
    locationId: "rack-1",
    zoneId: "zone-1",
    warehouseId: "warehouse-1",
  ...
  const bin = await service.createLocation({
    requestId: "req-bin",
    locationId: "bin-1",
    zoneId: "zone-1",
    warehouseId: "warehouse-1",
    ...
    occurredAt: "2026-09-04T12:03:00Z",
  });
