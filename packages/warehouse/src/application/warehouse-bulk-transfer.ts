...
export interface WarehouseImportIdGenerator {
  nextId(): string;
}
...
export interface WarehouseExportBatchSink {
  write(rows: readonly WarehouseExportRow[]): Promise<void>;
}
...
export class WarehouseBulkTransferService {
  ...
  async import(rows: readonly WarehouseTabularRow[], mapping: WarehouseImportColumnMap, context: WarehouseBulkContext, options: { readonly atomic: boolean }): Promise<WarehouseImportResult> {
    ...
    if (options.atomic && invalid.length > 0) {
      return Object.freeze({ importedCount: 0, failedCount: invalid.length, atomic: true, failures: Object.freeze(invalid) });
    }
  ...
  private async prepareRows(rows: readonly WarehouseTabularRow[], mapping: WarehouseImportColumnMap, context: WarehouseBulkContext): Promise<readonly PreparedRow[]> {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      prepared.push(await this.prepareRow(rows[index]!, index + 2, mapping, context));
    }
    const duplicateCodes = new Set<string>();
    const seen = new Set<string>();
    for (const entry of prepared) {
      if (!entry.state) continue;
      const key = normalizeWarehouseCode(entry.state.warehouse.code);
      if (seen.has(key)) duplicateCodes.add(key); else seen.add(key);
    }
    return Object.freeze(prepared.map((entry) => {
      if (!entry.state || !duplicateCodes.has(normalizeWarehouseCode(entry.state.warehouse.code))) return entry;
      return withIssue(entry, "warehouse.import.batch-duplicate", "Duplicate Warehouse code exists in the import batch.");
    }));
  }
  ...
  private async prepareRow(row: WarehouseTabularRow, rowNumber: number, mapping: WarehouseImportColumnMap, context: WarehouseBulkContext): Promise<PreparedRow> {
    try {
      ...
      return Object.freeze({
        rowNumber,
        state,
        preview: Object.freeze({ rowNumber, code: state.warehouse.code, title: state.warehouse.title, kind: state.warehouse.kind, valid: issues.length === 0, issues: Object.freeze(issues) }),
      });
    } catch (error) {
      return Object.freeze({
        rowNumber,
        ...
        preview: Object.freeze({
          ...
          kind: null,
          valid: false,
          issues: Object.freeze([{ code: "warehouse.import.invalid-row", message: error instanceof Error ? error.message : "Invalid Warehouse import row." }]),
        }),
      });
    }
  }
...
export class WarehouseInitialSetupService {
  ...
  async ensureDefault(context: WarehouseBulkContext, overrides: Partial<{ code: string; title: string; description: string | null }> = {}): Promise<{ readonly created: boolean; readonly warehouse: WarehouseDto | null }> {
    ...
    const warehouse = await this.service.create({
      requestId: context.requestId,
      warehouseId: this.ids.nextId(),
      companyId: context.companyId,
      code: overrides.code ?? defaultInitialWarehouse.code,
      title: overrides.title ?? defaultInitialWarehouse.title,
      description: overrides.description ?? null,
      kind: defaultInitialWarehouse.kind,
      organizationalScope: defaultInitialWarehouse.organizationalScope,
      externalIdentifiers: [],
      occurredAt: context.occurredAt,
    });
    await this.audit.record(Object.freeze({
      action: "warehouse.initial-setup",
      actorId: context.actorId,
      companyId: context.companyId,
...
async function mapRowToState(row: WarehouseTabularRow, mapping: WarehouseImportColumnMap, context: WarehouseBulkContext, warehouseId: string, branches: WarehouseBranchResolver): Promise<WarehousePersistenceState> {
  ...
  const base = createWarehouse({
    ...
    description: nullable(readMapped(row, mapping.description)),
    createdAt: context.occurredAt,
  });
  let warehouse = classifyWarehouse({ warehouse: base, kind });
  if (scope.mode === "branch") {
    ...
    warehouse = assignWarehouseOrganizationalScope({ warehouse, scope, branch });
  } else {
    warehouse = assignWarehouseOrganizationalScope({ warehouse, scope });
  }
  warehouse = status === "active" ? activateWarehouse(warehouse, context.occurredAt) : status === "inactive" ? deactivateWarehouse(warehouse, context.occurredAt) : archiveWarehouse(warehouse, context.occurredAt);
  const identifiers = createWarehouseIdentifierSnapshot(warehouse, parseExternalIdentifiers(readMapped(row, mapping.externalIdentifiers)));
  return Object.freeze({ warehouse, externalIdentifiers: identifiers.externalIdentifiers, version: 1 });
}

async function assertRepositoryAvailability(repository: WarehouseRepository, state: WarehousePersistenceState): Promise<void> {
  const byCode = await repository.findByCode(state.warehouse.companyId, state.warehouse.code);
  if (byCode) throw new Error("warehouse.import.duplicate-code");
  for (const identifier of state.externalIdentifiers) {
    const existing = await repository.findByExternalIdentifier(state.warehouse.companyId, identifier.namespace, identifier.value);
    if (existing) throw new Error("warehouse.import.duplicate-external-identifier");
  }
}

function replaceWarehouseId(state: WarehousePersistenceState, warehouseId: string): WarehousePersistenceState {
  return Object.freeze({
    ...state,
    warehouse: Object.freeze({ ...state.warehouse, warehouseId }),
  });
}
...
function readMapped(row: WarehouseTabularRow, column: string | undefined): string {
  if (!column) return "";
...
function withWriteFailure(preview: WarehouseImportPreviewRow, error: unknown): WarehouseImportPreviewRow {
  return Object.freeze({ ...preview, valid: false, issues: Object.freeze([...preview.issues, Object.freeze({ code: "warehouse.import.write-failed", message: error instanceof Error ? error.message : "Warehouse write failed." })]) });
}
function toExportRow(item: WarehouseDto): WarehouseExportRow {
  return Object.freeze({
    ...
    kind: item.kind,
    status: item.status,
    scopeMode: item.organizationalScope.mode,
    branchId: item.organizationalScope.mode === "branch" ? item.organizationalScope.branchId : "",
    externalIdentifiers: item.externalIdentifiers.map((entry) => `${entry.namespace}=${entry.value}`).join("|"),
