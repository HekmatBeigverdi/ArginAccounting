...
type ChronologyRow = InventoryKardexCursorPayload & { readonly quantityDelta: string };
...
function invalid(field: string): never {
  throw new InventoryApplicationError(INVENTORY_APPLICATION_ERROR_CODES.invalidRequest, field);
}
function assertLimit(limit: number): void {
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > MAX_LIMIT) invalid("limit");
}
...
function chronologyBefore(cursor: InventoryKardexCursorPayload): { sql: string; params: DatabaseValue[] } {
  return { sql: `(m.business_date < ? OR (m.business_date = ? AND m.business_order < ?) OR
    (m.business_date = ? AND m.business_order = ? AND m.document_id < ?) OR
    (m.business_date = ? AND m.business_order = ? AND m.document_id = ? AND m.line_id < ?) OR
    (m.business_date = ? AND m.business_order = ? AND m.document_id = ? AND m.line_id = ? AND m.movement_id < ?))`,
...
export class SqliteInventoryQuantityReportReader implements InventoryQuantityReportReader {
  ...
  /** Exact opening quantity before the current page's first eligible chronology point. Each SQL read is bounded. */
  private async openingQuantity(query: InventoryKardexReportQuery, pageCursor: InventoryKardexCursorPayload | null): Promise<string> {
    ...
    while (true) {
      ...
      if (pageCursor) { const before = chronologyBefore(pageCursor); clauses.push(before.sql); params.push(...before.params); }
      ...
      const rows = await this.database.query<{ business_date:string; business_order:number; document_id:string; line_id:string; movement_id:string; quantity_delta:string }>(
        `SELECT m.business_date,m.business_order,m.document_id,m.line_id,m.movement_id,m.quantity_delta
           FROM inventory_all_stock_movements m WHERE ${clauses.join(" AND ")}
          ORDER BY m.business_date,m.business_order,m.document_id,m.line_id,m.movement_id LIMIT ?`, [...params, CHUNK]);
  ...
  async readKardex(query: InventoryKardexReportQuery): Promise<InventoryKardexReport> {
    ...
    const base = stockKeyClauses(key); const clauses = [base.sql]; const params: DatabaseValue[] = [...base.params];
    if (query.businessDateFrom) { clauses.push("m.business_date>=?"); params.push(query.businessDateFrom); }
    if (query.businessDateTo) { clauses.push("m.business_date<=?"); params.push(query.businessDateTo); }
    ...
    const rows = await this.database.query<MovementRow>(
      `SELECT m.*,d.document_number,d.document_type,l.position AS line_position,
    ...
    const hasMore = rows.length > query.limit; const pageRows = rows.slice(0,query.limit);
    const openingQuantity = await this.openingQuantity(query,cursor); let running=openingQuantity,incoming="0",outgoing="0";
    const entries: InventoryKardexReportEntry[] = pageRows.map((row) => {
      const fact=movement(row); running=addInventoryStockQuantities(running,fact.quantityDelta);
      if(fact.quantityDelta.startsWith("-")) outgoing=addInventoryStockQuantities(outgoing,positiveMagnitude(fact.quantityDelta));
      else incoming=addInventoryStockQuantities(incoming,fact.quantityDelta);
      return Object.freeze({ movement:fact, incomingQuantity:fact.quantityDelta.startsWith("-")?"0":fact.quantityDelta,
        outgoingQuantity:fact.quantityDelta.startsWith("-")?positiveMagnitude(fact.quantityDelta):"0", runningQuantity:running,
        source:Object.freeze({ documentId:row.document_id,documentNumber:row.document_number,documentType:row.document_type,lineId:row.line_id,
          linePosition:row.line_position,sourceSystem:row.source_system,sourceDocumentType:row.source_document_type,
          sourceDocumentId:row.source_document_id,sourceLineId:row.source_line_id }) });
    });
    const last=pageRows.at(-1);
    return Object.freeze({ stockKey:key,openingQuantity,incomingQuantity:incoming,outgoingQuantity:outgoing,closingQuantity:running,
      entries:Object.freeze(entries),nextCursor:hasMore&&last?encodeCursor(cursorFromRow(last)):null });
  }
  ...
  async readBalances(query: InventoryQuantityBalanceReportQuery): Promise<InventoryQuantityBalanceReport> {
    ...
    const clauses=["b.company_id=?"],params:DatabaseValue[]=[query.companyId];
    ...
    if(!query.includeZero) clauses.push("b.quantity NOT IN ('0','0.0','-0','-0.0')");
    if(query.cursor){clauses.push("b.stock_key > ?");params.push(query.cursor);}
    const rows=await this.database.query<BalanceRow>(
      `SELECT b.stock_key,b.company_id,b.product_id,b.warehouse_id,b.zone_id,b.location_id,b.quantity,b.last_movement_id,
              p.code AS product_code,p.title AS product_title,w.code AS warehouse_code,w.title AS warehouse_title,
              z.code AS zone_code,z.title AS zone_title,l.code AS location_code,l.title AS location_title
         FROM inventory_stock_balances b JOIN products p ON p.company_id=b.company_id AND p.id=b.product_id
    ...
    const hasMore=rows.length>query.limit,pageRows=rows.slice(0,query.limit);
    return Object.freeze({items:Object.freeze(pageRows.map((row)=>Object.freeze({stockKey:Object.freeze({companyId:row.company_id,productId:row.product_id,
      warehouseId:row.warehouse_id,zoneId:row.zone_id,locationId:row.location_id}),quantity:normalizeInventoryQuantity(row.quantity),
      productCode:row.product_code,productTitle:row.product_title,warehouseCode:row.warehouse_code,warehouseTitle:row.warehouse_title,
      zoneCode:row.zone_code,zoneTitle:row.zone_title,locationCode:row.location_code,locationTitle:row.location_title,lastMovementId:row.last_movement_id}))),
      nextCursor:hasMore&&pageRows.length?pageRows[pageRows.length-1]!.stock_key:null});
  }
}
