...
function decodeCursor(value: string | null | undefined): InventoryKardexCursorPayload | null {
  ...
  try {
    ...
    if (typeof parsed.businessDate !== "string" || !Number.isSafeInteger(parsed.businessOrder) ||
      typeof parsed.documentId !== "string" || typeof parsed.lineId !== "string" || typeof parsed.movementId !== "string") {
...
function chronologyAfter(cursor: InventoryKardexCursorPayload): { sql: string; params: DatabaseValue[] } {
  return {
    sql: `(m.business_date > ? OR
      (m.business_date = ? AND m.business_order > ?) OR
    ...
    params: [
      cursor.businessDate,
      cursor.businessDate, cursor.businessOrder,
      cursor.businessDate, cursor.businessOrder, cursor.documentId,
      cursor.businessDate, cursor.businessOrder, cursor.documentId, cursor.lineId,
      cursor.businessDate, cursor.businessOrder, cursor.documentId, cursor.lineId, cursor.movementId,
    ],
  };
}
...
function positiveMagnitude(value: string): string {
...
export class SqliteInventoryQuantityReportReader implements InventoryQuantityReportReader {
  ...
  private async sumBefore(query: InventoryKardexReportQuery, cursor: InventoryKardexCursorPayload | null): Promise<string> {
    const key = createInventoryStockKey({
      companyId: query.companyId,
      productId: query.stockKey.productId,
      warehouse: {
        warehouseId: query.stockKey.warehouseId,
        zoneId: query.stockKey.zoneId,
        locationId: query.stockKey.locationId,
      },
    });
    ...
    while (true) {
      ...
      if (!query.businessDateFrom && cursor) {
        const before = chronologyBefore(cursor); clauses.push(before.sql); params.push(...before.params);
      } else if (query.businessDateFrom && cursor) {
        const before = chronologyBefore(cursor); clauses.push(`(m.business_date < ? OR (m.business_date >= ? AND ${before.sql}))`);
        params.push(query.businessDateFrom, query.businessDateFrom, ...before.params);
      }
      const rows = await this.database.query<{ quantity_delta: string }>(
        `SELECT m.quantity_delta FROM inventory_all_stock_movements m
  ...
  async readKardex(query: InventoryKardexReportQuery): Promise<InventoryKardexReport> {
    ...
    const key = createInventoryStockKey({ companyId: query.companyId, productId: query.stockKey.productId, warehouse: query.stockKey });
    ...
    const rows = await this.database.query<MovementRow>(
      `SELECT m.*,d.document_number,d.document_type,l.position AS line_position,
              COALESCE(l.source_system,d.source_system) AS source_system,
              COALESCE(l.source_document_type,d.source_document_type) AS source_document_type,
              COALESCE(l.source_document_id,d.source_document_id) AS source_document_id,
              COALESCE(l.source_line_id,d.source_line_id) AS source_line_id
    ...
    const entries: InventoryKardexReportEntry[] = pageRows.map((row) => {
      ...
      return Object.freeze({
        movement: fact,
        incomingQuantity: fact.quantityDelta.startsWith("-") ? "0" : fact.quantityDelta,
        outgoingQuantity: fact.quantityDelta.startsWith("-") ? positiveMagnitude(fact.quantityDelta) : "0",
    ...
    return Object.freeze({
      ...
      nextCursor: hasMore && last ? encodeCursor({ businessDate: last.business_date, businessOrder: last.business_order, documentId: last.document_id, lineId: last.line_id, movementId: last.movement_id }) : null,
    });
  }
  ...
  async readBalances(query: InventoryQuantityBalanceReportQuery): Promise<InventoryQuantityBalanceReport> {
    ...
    add("b.product_id=?", query.productId);
    add("b.warehouse_id=?", query.warehouseId);
    add("b.zone_id=?", query.zoneId);
    add("b.location_id=?", query.locationId);
    ...
    const rows = await this.database.query<BalanceRow>(
      `SELECT b.stock_key,b.company_id,b.product_id,b.warehouse_id,b.zone_id,b.location_id,b.quantity,b.last_movement_id,
         ...
         JOIN products p ON p.company_id=b.company_id AND p.id=b.product_id
    ...
    const pageRows = rows.slice(0, query.limit);
    return Object.freeze({
      items: Object.freeze(pageRows.map((row) => Object.freeze({
        stockKey: Object.freeze({ companyId: row.company_id, productId: row.product_id, warehouseId: row.warehouse_id, zoneId: row.zone_id, locationId: row.location_id }),
