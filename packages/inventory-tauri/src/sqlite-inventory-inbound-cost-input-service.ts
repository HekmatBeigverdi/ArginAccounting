...
export type InventoryInboundCostMethod = "fifo" | "moving_average";
...
function calculateRoundedTotalCost(quantity: string, unitCost: string): number {
  ...
  const denominator = 10n ** BigInt(quantityDecimal.scale + unitCostDecimal.scale);
...
function candidateFromRow(row: CandidateRow): InventoryInboundCostCandidate {
  return Object.freeze({
    ...
    policyReady: row.method !== null && row.strategy_version !== null,
  });
}
...
const candidateSql = `
  ...
  LEFT JOIN inventory_valuation_policies p
    ON p.policy_id=(
      ...
      WHERE p2.company_id=m.company_id
        AND p2.effective_from<=m.business_date
  ...
  async setManualCost(
    ...
    if (replay) {
      ...
      const valuation = await this.db.queryOne<{ found: number }>(
        `SELECT 1 AS found FROM inventory_valuation_entries WHERE company_id=? AND movement_id=?`,
        [companyId, movementId],
      );
      return Object.freeze({
        ...
        valuationCreated: Boolean(valuation),
      });
    }
      ...
      if (candidate.policyReady && candidate.method && candidate.strategyVersion) {
        ...
        if (!existingEntry) {
          await session.execute(
            ...
            SELECT ?,m.company_id,m.product_id,m.movement_id,m.document_id,m.line_id,
              m.reversal_of_movement_id,m.transfer_id,'inbound',?,?,?,m.warehouse_id,
              m.zone_id,m.location_id,m.business_date,m.business_order,?,?,?,
              'resolved',NULL,?,1
            FROM inventory_all_stock_movements m
            WHERE m.company_id=? AND m.movement_id=?`,
        ...
        if (candidate.method === "fifo") {
          await session.execute(
            ...
            SELECT ?,m.company_id,m.product_id,m.movement_id,?,'fifo',?,?,m.warehouse_id,
