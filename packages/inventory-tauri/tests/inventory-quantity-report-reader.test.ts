...
function movementRow(input: { movementId: string; date: string; order: number; delta: string }) {
  return {
...
function fakeExecutor(): DatabaseExecutor {
  let mainCall = 0;
  let prefixCall = 0;
  const opening = movementRow({ movementId: "opening", date: "2026-01-01", order: 1, delta: "5" });
  ...
  database.query = async function <T>(sql: string, parameters) {
    if (sql.includes("FROM inventory_stock_balances b")) {
      return [{
        ...
        zone_id: null, location_id: null, quantity: exact, last_movement_id: "movement-1",
        product_code: "P-1", product_title: "کالا", warehouse_code: "W-1", warehouse_title: "انبار",
        zone_code: null, zone_title: null, location_code: null, location_title: null,
      }] as T[];
    }
