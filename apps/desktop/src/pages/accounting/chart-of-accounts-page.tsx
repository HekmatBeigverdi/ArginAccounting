...
type CodingLengthField = keyof Pick<
  AccountCodingSettings,
  "groupCodeLength" | "generalCodeLength" | "subsidiaryCodeLength"
>;

const codingLengthFields: readonly [CodingLengthField, string][] = [
...
function childAccountLevel(parent?: Account): AccountLevel {
  if (!parent) return "group";
  return parent.level === "group" ? "general" : "subsidiary";
}

function accountLevelInitial(level: AccountLevel): string {
  if (level === "group") return "گ";
  if (level === "general") return "ک";
  return "م";
}

function accountLevelTone(level: AccountLevel): "info" | "warning" | "neutral" {
  if (level === "group") return "info";
  ...
  const visit = (parentId: string | null, depth: number): void => {
    for (const account of byParent.get(parentId) ?? []) {
      ...
      const hasChildren = children.some(
        (child) => !filtering || visibleIds.has(child.id),
      );
...
export function ChartOfAccountsPage() {
  ...
  const groupCount = accounts.filter(
    (account) => account.level === "group",
  ).length;
  const generalCount = accounts.filter(
    (account) => account.level === "general",
  ).length;
  const subsidiaryCount = accounts.filter(
    (account) => account.level === "subsidiary",
  ).length;
  ...
  function startCreate(parent?: Account): void {
    const level = childAccountLevel(parent);
    setEditing(null);
    setDraft({
      ...emptyDraft,
      level,
  ...
  async function toggleStatus(account: Account): Promise<void> {
    ...
    try {
      ...
      setMessage(
        nextStatus === "active" ? "حساب فعال شد." : "حساب غیرفعال شد.",
      );
      await reload();
    } catch (reason) {
      setError(getAccountingErrorMessage(reason));
    ...
    <Page className="accounting-workspace coa-page" lang="fa" dir="rtl">
      <header className="accounting-workspace__header">
        <div>
          ...
          <h1>درخت حساب‌ها</h1>
          <p>
            ساختار گروه، کل و معین را با نمای سلسله‌مراتبی یکپارچه مدیریت کنید.
          </p>
        </div>
        <Button
          type="button"
      ...
      <Toolbar className="accounting-workspace__toolbar coa-toolbar">
        ...
        <Field label="جست‌وجو" className="coa-toolbar__search">
          <Input
            value={search}
            placeholder="کد، عنوان فارسی یا انگلیسی"
        ...
        <Field label="وضعیت" className="coa-toolbar__status">
          <Select
            value={statusFilter}
            onChange={(event) =>
              setStatusFilter(event.target.value as "" | Account["status"])
            }
          >
      ...
      <div className="coa-summary" aria-label="خلاصه ساختار حساب‌ها">
        <Panel className="coa-summary__item">
          <span>کل حساب‌ها</span>
          <strong>{accounts.length.toLocaleString("fa-IR")}</strong>
        </Panel>
        <Panel className="coa-summary__item coa-summary__item--group">
          <span>گروه</span>
          <strong>{groupCount.toLocaleString("fa-IR")}</strong>
        </Panel>
        <Panel className="coa-summary__item coa-summary__item--general">
          <span>کل</span>
          <strong>{generalCount.toLocaleString("fa-IR")}</strong>
        </Panel>
        <Panel className="coa-summary__item coa-summary__item--subsidiary">
          <span>معین</span>
          <strong>{subsidiaryCount.toLocaleString("fa-IR")}</strong>
        </Panel>
      </div>
        ...
        <Panel className="coa-tree-card">
          <div className="coa-tree-card__title">
            <div>
              ...
              <p>
                شاخه‌ها را باز یا بسته کنید؛ در جست‌وجو مسیر والد حساب‌های منطبق
                حفظ می‌شود.
              </p>
            </div>
            <div className="coa-tree-card__controls">
              <Badge tone="info">
                {treeRows.length.toLocaleString("fa-IR")} ردیف
              </Badge>
              <Button type="button" compact onClick={expandAll}>
                بازکردن همه
              </Button>
              <Button type="button" compact onClick={collapseAll}>
                بستن همه
              </Button>
            </div>
          </div>
          <div className="coa-tree" role="tree" aria-label="درخت حساب‌ها">
            {treeRows.map(({ account, depth, hasChildren, isExpanded }) => (
              <article
                ...
                style={{ "--coa-depth": depth } as AccountTreeStyle}
              >
                <div className="coa-tree__identity">
                  ...
                  {hasChildren ? (
                    <button
                      ...
                      aria-label={
                        isExpanded
                          ? `بستن شاخه ${account.name}`
                          : `باز کردن شاخه ${account.name}`
                      }
                    ...
                    <span className="coa-tree__leaf" aria-hidden="true">
                      •
                    </span>
                  )}
                  <span
                    className={`coa-tree__level-marker coa-tree__level-marker--${account.level}`}
                    aria-hidden="true"
                  >
                    {accountLevelInitial(account.level)}
                  </span>
                  <div className="coa-tree__title">
                    ...
                    {account.englishName ? (
                      <small dir="ltr">{account.englishName}</small>
                    ) : null}
                  </div>
                </div>
                <div className="coa-tree__meta">
                  <Badge tone={accountLevelTone(account.level)}>
                    {accountLevelLabels[account.level]}
                  </Badge>
                  <span>
                    {account.normalBalance === "debit" ? "بدهکار" : "بستانکار"}
                  </span>
                  {account.postingAllowed ? (
                    <Badge tone="success">قابل ثبت</Badge>
                  ) : (
                    <span className="coa-tree__posting-muted">غیرقابل ثبت</span>
                  )}
                  <Badge
                ...
                <div className="coa-tree__actions">
                  ...
                  {can(chartOfAccountsPermissions.changeStatus) ? (
                    ...
                    >
                      {account.status === "active" ? "غیرفعال" : "فعال"}
                    </Button>
                  ) : null}
            ...
            {treeRows.length === 0 ? (
              <div className="coa-empty">
                حسابی مطابق فیلترهای فعلی وجود ندارد.
              </div>
            ) : null}
          </div>
        </Panel>
            ...
            >
              <div className="coa-card__title">
                <div>
                  ...
                  <p>
                    {editing === null
                      ? "حساب جدید را در سطح مناسب ایجاد کنید."
                      : "مشخصات حساب انتخاب‌شده را اصلاح کنید."}
                  </p>
                </div>
                {editing ? (
                  <Button
              ...
              <div className="coa-fields-row">
                ...
                <Field label="مانده عادی">
                  <Select
                    value={draft.normalBalance}
                    onChange={(event) =>
                      setDraft((current) => ({
                        ...current,
                        normalBalance: event.target.value as NormalBalance,
                      }))
                    }
                  >
                    <option value="debit">بدهکار</option>
              ...
              <Field label="صورت مالی">
                <Select
                  value={draft.statementType}
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      statementType: event.target.value as AccountStatementType,
                    }))
                  }
                >
            ...
            <Panel className="coa-card">
              ...
              >
                <div className="coa-card__title">
                  <div>
                    <h2>تنظیمات کدینگ</h2>
                    <p>طول کدها و قواعد سلسله‌مراتبی شرکت</p>
                  </div>
                  <Badge>نسخه {settings.version.toLocaleString("fa-IR")}</Badge>
                </div>
                <div className="coa-settings-grid">
                  {codingLengthFields.map(([field, label]) => (
                ...
                <Button
                  type="submit"
                  disabled={
                    busy || !can(chartOfAccountsPermissions.manageSettings)
                  }
                >
                  ذخیره تنظیمات
                </Button>
