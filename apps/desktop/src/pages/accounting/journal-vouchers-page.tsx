...
export function JournalVouchersPage() {
  ...
  const reloadList = useCallback(async () => {
    ...
    try {
      const result = await journals.search({ companyId, ...(search.trim() ? { text: search.trim() } : {}), ...(dateFrom ? { dateFrom } : {}), ...(dateTo ? { dateTo } : {}), page: 1, pageSize: 100 });
  ...
  const loadReferenceData = useCallback(async (targetCompanyId: string) => {
    ...
    const [accountOptions, branchOptions] = await Promise.all([journals.listPostingAccounts(targetCompanyId), journals.listBranches(targetCompanyId)]);
  ...
  const loadDimensionsForLine = useCallback(async (line: VoucherLineDraft, voucherDate = draft.voucherDate) => {
    ...
    const model = await journals.loadDimensionSelector({ companyId, accountId: line.accountId, documentDate: voucherDate, assignments: line.assignments });
  ...
  async function openVoucher(id: string): Promise<void> {
    ...
    try { setSelected(await journals.get({ companyId, voucherId: id })); }
    catch (reason) { showError(reason); }
    finally { setBusy(false); }
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    setBusy(true); clearError(); setMessage("");
    try {
      const lines = draft.lines.map((line, index) => ({ id: line.id ?? undefined, order: index + 1, accountId: line.accountId, description: line.description || null, debit: parseRialInput(line.debit), credit: parseRialInput(line.credit), dimensionAssignments: line.assignments }));
  ...
  return (
    <section className="accounting-workspace journal-page" lang="fa" dir="rtl">
      <header className="accounting-workspace__header journal-page__header"><div><p className="accounting-workspace__eyebrow">حسابداری / اسناد حسابداری</p><h1>اسناد حسابداری</h1><p>ثبت، ویرایش و کنترل سند دوبل پیش‌نویس</p></div><div className="journal-page__header-actions"><button type="button" onClick={() => void reloadList()} disabled={busy}>تازه‌سازی</button><button className="journal-button journal-button--primary" type="button" disabled={!can(journalVoucherPermissions.create)} onClick={startNew}>+ سند جدید</button></div></header>
      <section className="journal-searchbar" aria-label="جستجوی اسناد"><label>شرکت<select value={companyId} onChange={(event) => { setCompanyId(event.target.value); setSelected(null); startNew(); }}>{companies.map((company) => <option key={company.id} value={company.id}>{company.legalName}</option>)}</select></label><label className="journal-searchbar__text">جست‌وجو<input value={search} placeholder="شماره، مرجع یا شرح سند" onChange={(event) => setSearch(event.target.value)} /></label><label>از تاریخ<PersianDatePicker value={dateFrom} onChange={setDateFrom} ariaLabel="از تاریخ" placeholder="از تاریخ شمسی" /></label><label>تا تاریخ<PersianDatePicker value={dateTo} onChange={setDateTo} ariaLabel="تا تاریخ" placeholder="تا تاریخ شمسی" /></label><button type="button" onClick={() => void reloadList()} disabled={busy}>جست‌وجو</button></section>
      {companies.length === 0 && !busy && <p className="journal-notice">ابتدا یک شرکت و سال مالی ایجاد کنید.</p>}{error && <p className="journal-alert journal-alert--error" role="alert">{error}</p>}{technicalError && <details className="journal-technical-error"><summary>جزئیات فنی</summary><code dir="ltr">{technicalError}</code></details>}{message && <p className="journal-alert journal-alert--success" role="status">{message}</p>}
      <div className="journal-layout"><aside className="journal-list-card"><div className="journal-card-title"><div><h2>فهرست اسناد</h2><small>آخرین اسناد شرکت جاری</small></div><span className="journal-count-badge">{items.length.toLocaleString("fa-IR")}</span></div><div className="journal-list">{items.map((item) => <button type="button" className="journal-list__item" key={item.id} onClick={() => void openVoucher(item.id)}><span className="journal-list__number"><b dir="ltr">{item.number}</b><small>{formatJournalVoucherDate(item.voucherDate)}</small></span><span>{item.description || item.reference || "بدون شرح"}</span><strong>{formatJournalRials(item.totalDebit.amount)}</strong></button>)}{items.length === 0 && <p className="journal-empty">سندی برای نمایش وجود ندارد.</p>}</div></aside>
        <main className="journal-workspace">{selected ? <VoucherDetail voucher={selected} accounts={accounts} canEdit={can(journalVoucherPermissions.updateDraft)} canDelete={can(journalVoucherPermissions.deleteDraft)} onEdit={() => void startEdit()} onDelete={() => void removeSelected()} onClose={() => setSelected(null)} /> : <form className="journal-editor" onSubmit={(event) => void submit(event)}>
          <section className="journal-document-card"><div className="journal-section-heading"><div><p className="journal-page__eyebrow">{draft.voucherId ? "ویرایش پیش‌نویس" : "سند جدید"}</p><h2>اطلاعات سند</h2></div><div className="journal-document-status"><span className="journal-status-badge">پیش‌نویس</span>{draft.voucherId && <span>نسخه {draft.version?.toLocaleString("fa-IR")}</span>}</div></div><div className="journal-document-grid"><label>شرکت<input readOnly value={currentCompany?.legalName ?? "—"} /></label><label>شماره سند<input readOnly value={draft.voucherId ? "حفظ شماره فعلی" : "خودکار هنگام ذخیره"} /></label><label>تاریخ سند<PersianDatePicker value={draft.voucherDate} onChange={(value) => { void changeVoucherDate(value); }} ariaLabel="تاریخ سند" /></label><label>شعبه<select value={draft.branchId} onChange={(event) => setDraft((current) => ({ ...current, branchId: event.target.value }))}><option value="">بدون شعبه</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.code} — {branch.name}</option>)}</select></label><label>شماره مرجع<input value={draft.reference} maxLength={100} placeholder="اختیاری" onChange={(event) => setDraft((current) => ({ ...current, reference: event.target.value }))} /></label><label>منبع<input readOnly value="ثبت دستی" /></label><label className="journal-document-grid__description">شرح سند<input value={draft.description} placeholder="شرح کلی سند حسابداری" onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} /></label></div></section>
          <section className="journal-lines-card"><div className="journal-section-heading journal-section-heading--lines"><div><h2>آرتیکل‌های سند</h2><p>حساب، شرح، ابعاد و مبالغ هر ردیف را وارد کنید.</p></div><button type="button" onClick={() => setDraft((current) => ({ ...current, lines: [...current.lines, emptyLine()] }))}>+ ردیف</button></div><div className="journal-entry-table-wrap"><table className="journal-entry-table"><colgroup><col className="journal-col-row" /><col className="journal-col-account" /><col className="journal-col-description" /><col className="journal-col-dimension" /><col className="journal-col-amount" /><col className="journal-col-amount" /><col className="journal-col-actions" /></colgroup><thead><tr><th>ردیف</th><th>حساب</th><th>شرح</th><th>ابعاد حسابداری</th><th>بدهکار (ریال)</th><th>بستانکار (ریال)</th><th>عملیات</th></tr></thead><tbody>{draft.lines.map((line, index) => <tr key={line.key}><td className="journal-entry-table__row-number">{(index + 1).toLocaleString("fa-IR")}</td><td><select value={line.accountId} onChange={(event) => { const accountId = event.target.value; updateLine(line.key, { accountId, assignments: [] }); if (accountId) void loadDimensionsForLine({ ...line, accountId, assignments: [] }); }}><option value="">انتخاب حساب</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.code} — {account.name}</option>)}</select></td><td><input value={line.description} onChange={(event) => updateLine(line.key, { description: event.target.value })} /></td><td><DimensionCell line={line} selector={selectors[line.key]} onChange={updateAssignment} /></td><td><input className="journal-money-input" inputMode="numeric" dir="ltr" value={line.debit} onChange={(event) => updateLine(line.key, { debit: event.target.value, credit: event.target.value ? "" : line.credit })} /></td><td><input className="journal-money-input" inputMode="numeric" dir="ltr" value={line.credit} onChange={(event) => updateLine(line.key, { credit: event.target.value, debit: event.target.value ? "" : line.debit })} /></td><td className="journal-entry-table__actions"><button type="button" disabled={draft.lines.length <= 2} onClick={() => setDraft((current) => ({ ...current, lines: current.lines.filter((item) => item.key !== line.key) }))}>حذف</button></td></tr>)}</tbody></table></div><div className="journal-summary-grid"><div className="journal-summary-panel"><span>جمع بدهکار</span><strong>{formatJournalRials(totals.debit)}</strong></div><div className="journal-summary-panel"><span>جمع بستانکار</span><strong>{formatJournalRials(totals.credit)}</strong></div><div className="journal-summary-panel"><span>اختلاف</span><strong>{formatJournalRials(Math.abs(difference))}</strong></div><div className={`journal-balance-state ${difference === 0 && totals.debit > 0 ? "journal-balance-state--ok" : "journal-balance-state--warning"}`}>{difference === 0 && totals.debit > 0 ? "سند تراز است" : "جمع بدهکار و بستانکار باید برابر باشد"}</div></div></section>
...
function DimensionCell({ line, selector, onChange }: { line: VoucherLineDraft; selector?: AccountingDimensionSelectorModel; onChange: (line: VoucherLineDraft, field: AccountingDimensionSelectorField, memberIds: readonly string[]) => void }) {
  ...
  return <div className="journal-dimension-cell">{selector.fields.map((field) => { const assignment = line.assignments.find((item) => item.dimensionTypeId === field.dimensionTypeId); const selectedIds = assignment?.memberIds ?? []; if (field.allowMultipleMembers) return <label key={field.dimensionTypeId}><small>{field.dimensionTypeName}{field.requirement === "required" ? " *" : ""}</small><select multiple value={[...selectedIds]} onChange={(event) => onChange(line, field, Array.from(event.currentTarget.selectedOptions).map((option) => option.value))}>{field.members.map((member) => <option key={member.id} value={member.id}>{member.code} — {member.name}</option>)}</select></label>; return <label key={field.dimensionTypeId}><small>{field.dimensionTypeName}{field.requirement === "required" ? " *" : ""}</small><select value={selectedIds[0] ?? ""} onChange={(event) => onChange(line, field, event.target.value ? [event.target.value] : [])}><option value="">انتخاب کنید</option>{field.members.map((member) => <option key={member.id} value={member.id}>{member.code} — {member.name}</option>)}</select></label>; })}</div>;
}
