...
import { codingTemplateErrorDetails, codingTemplateLabel, formatJalaliDate, type CodingTemplateErrorDetails } from "../../features/accounting/coding-templates-presenter";
...
export function CodingTemplatesPage() {
  ...
  const [error, setError] = useState<CodingTemplateErrorDetails | null>(null);
  const [applyRequestKey, setApplyRequestKey] = useState("");
  ...
  useEffect(() => { void getDesktopDatabase().then((db) => new SqliteCompanyRepository(db).findAll()).then((values) => { setCompanies(values); setCompanyId((id) => id || values[0]?.id || ""); }).catch((reason) => setError(codingTemplateErrorDetails(reason))); }, []);
  useEffect(() => { void load().catch((reason) => setError(codingTemplateErrorDetails(reason))); }, [load]);
  useEffect(() => { setPreview(null); setApplyRequestKey(""); setUpgrade(null); setConfirmed(false); if (!selectedId) { setVersions([]); return; } void codingTemplates.searchVersions(selectedId).then((items) => { setVersions(items); setVersionId(String(items[0]?.version.id ?? "")); }).catch((reason) => setError(codingTemplateErrorDetails(reason))); }, [codingTemplates, selectedId]);

  async function run(action: () => Promise<void>, success?: string) { setBusy(true); setError(null); setMessage(""); try { await action(); if (success) setMessage(success); } catch (reason) { setError(codingTemplateErrorDetails(reason)); } finally { setBusy(false); } }
  async function createPreview() { await run(async () => { const value = await codingTemplates.preview(companyId, versionId); setPreview(value); setApplyRequestKey(crypto.randomUUID()); setConfirmed(false); setTab("preview"); }); }
  async function apply() { if (!preview || !selected || !confirmed || !applyRequestKey) return; await run(async () => { await codingTemplates.apply({ companyId, templateId: String(selected.id), versionId, baselineFingerprint: preview.baselineFingerprint, requestKey: applyRequestKey }); setPreview(null); setApplyRequestKey(""); setConfirmed(false); await load(); }, "الگوی کدینگ با موفقیت روی شرکت اعمال شد."); }
  ...
  return <main className="coding-templates" dir="rtl">
    ...
    {error && <div className="coding-templates__notice coding-templates__notice--error" role="alert"><strong>{error.summary}</strong><div dir="ltr"><small>Technical error: <code>{error.technical}</code></small></div></div>}{message && <div className="coding-templates__notice" role="status">{message}</div>}
