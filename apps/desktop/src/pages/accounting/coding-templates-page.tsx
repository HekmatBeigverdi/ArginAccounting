...
import { codingTemplateErrorMessage, codingTemplateIssueMessage, codingTemplateLabel, formatJalaliDate } from "../../features/accounting/coding-templates-presenter";
...
export function CodingTemplatesPage() {
  ...
  return <main className="coding-templates" dir="rtl">
    ...
    {tab === "preview" && <section className="coding-templates__panel"><h2>نتیجه پیش‌نمایش</h2>{preview ? <><div className="coding-templates__summary"><span>ایجاد <b>{preview.summary.create}</b></span><span>سازگار <b>{preview.summary.compatibleExisting}</b></span><span>تعارض <b>{preview.summary.conflict}</b></span><span>نامعتبر <b>{preview.summary.invalid}</b></span></div>{preview.issues.map((issue, index) => <p className="coding-templates__issue" key={`${issue.logicalKey}-${index}`}>{codingTemplateIssueMessage(issue.code)} <code>{issue.logicalKey}</code></p>)}<label className="coding-templates__confirm"><input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)}/> پیش‌نمایش را بررسی کردم و اعمال این نسخه را تأیید می‌کنم.</label><button disabled={busy || !preview.canApply || !confirmed || !can(codingTemplatePermissions.apply)} onClick={() => void apply()}>اعمال اتمیک الگو</button></> : <p className="muted">برای شروع، از کاتالوگ یک نسخه را پیش‌نمایش کنید.</p>}</section>}
