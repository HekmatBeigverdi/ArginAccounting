...
export function jalaliToGregorian(value: string): string {
  ...
  if(jm<1||jm>12||jd<1||jd>maxDay) throw new Error("تاریخ شمسی معتبر نیست.");
  const g=d2g(j2d(jy,jm,jd));
  const result = `${g.gy}-${String(g.gm).padStart(2,"0")}-${String(g.gd).padStart(2,"0")}`;
  if (gregorianToJalali(result) !== `${jy}/${String(jm).padStart(2,"0")}/${String(jd).padStart(2,"0")}`) throw new Error("تاریخ شمسی معتبر نیست.");
  return result;
}
