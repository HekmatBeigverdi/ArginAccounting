...
export function InventoryValuationWorkspacePage() {
  ...
  const [inboundCostCandidates, setInboundCostCandidates] = useState<
    readonly InventoryInboundCostCandidate[]
  >([]);
      ...
      .catch((caughtError) =>
        setError(
          caughtError instanceof Error
            ? caughtError.message
            : "راه‌اندازی ارزش‌گذاری ناموفق بود.",
        ),
      );
  }, [actor]);
  ...
  function createUnresolvedQuery(companyId: string) {
    return {
      companyId,
  ...
  async function refreshUnresolvedWorkspace(): Promise<void> {
  ...
  async function correctInboundCost(
    row: InventoryResolvedInboundCost,
    unitCost: string,
    reason: string,
  ): Promise<void> {
    if (!services || !activeContext.companyId) return;
    ...
    try {
      ...
      const message =
        caughtError instanceof Error
          ? caughtError.message
          : "اصلاح بهای ورودی ناموفق بود.";
      setError(
        message === "VALUATION_COST_INPUT_CORRECTION_RECALCULATION_REQUIRED"
    ...
    <Page className="valuation-page" dir="rtl">
      ...
      {activeTab === "unresolved" && (
        <>
          ...
          />
          <ValuationRegisteredCostPanel
            rows={resolvedInboundCosts}
            canCorrect={services?.canCorrectCostInput ?? false}
            saving={savingCost}
            onCorrectCost={correctInboundCost}
          />
