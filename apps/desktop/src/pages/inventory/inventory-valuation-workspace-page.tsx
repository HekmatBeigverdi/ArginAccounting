...
export function InventoryValuationWorkspacePage() {
  ...
  useEffect(() => {
    void getDesktopDatabase()
      ...
      .catch((e) =>
        setError(e instanceof Error ? e.message : "راه‌اندازی ارزش‌گذاری ناموفق بود."),
      );
  }, [actor]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((e) =>
        setError(e instanceof Error ? e.message : "بارگذاری انتخابگرها ناموفق بود."),
      );
      ...
      if (nextTab === "overview") {
        ...
        setKardex(
          await services.readKardex({
            ...
            productId,
            warehouseId,
            businessDateFrom: dateFrom || null,
            businessDateTo: dateTo || null,
            cursor,
            limit: 100,
          }),
        );
  ...
  async function setInboundCost(
    ...
    try {
      ...
      setSuccess(
        result.valuationCreated
          ? `بهای ورودی ثبت و ارزش‌گذاری شد: ${amount} ${result.currency}`
          : `بهای ورودی ثبت شد: ${amount} ${result.currency}. برای ایجاد ارزش‌گذاری، سیاست ارزش‌گذاری مؤثر در تاریخ سند را تعیین کنید.`,
      );
  ...
  if (services && !services.canView) {
    return (
      <Page>
        <Feedback tone="error">برای مشاهده ارزش‌گذاری موجودی مجوز کافی ندارید.</Feedback>
      </Page>
    );
  }
    ...
    <Page className="valuation-page" dir="rtl">
      <header className="valuation-head">
        <div>
          ...
          <p>ارزش ریالی، کاردکس مالی، FIFO، موارد حل‌نشده و تاریخچه سیاست.</p>
        </div>
      ...
      <p className="valuation-note">
        <strong>پل آرگین:</strong> Movement، Cost Input و Policy داده اصلی‌اند؛ جمع گزارش و مانده ریالی خروجی بازسازی‌پذیر هستند.
      </p>
        ...
        {activeTab === "kardex" && (
          <label>
            ...
            <PersianDatePicker
              value={dateTo}
              onChange={setDateTo}
              ariaLabel="تا تاریخ کاردکس ریالی"
            />
          </label>
        )}
      ...
      {activeTab === "unresolved" && (
        <>
          <ValuationInboundCostPanel
            ...
            saving={savingCost}
            onSetCost={setInboundCost}
          />
          {unresolved && <ValuationUnresolvedPanel report={unresolved} />}
        </>
      )}
      {activeTab === "policy" && (
        <ValuationPolicyPanel rows={policies} readOnly={!services?.canManagePolicy} />
      )}
