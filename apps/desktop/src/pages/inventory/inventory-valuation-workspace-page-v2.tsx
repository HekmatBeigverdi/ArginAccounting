...
const todayInJalali = () => gregorianToJalali(new Date().toISOString().slice(0, 10));
...
export function InventoryValuationWorkspacePage() {
  ...
  useEffect(() => {
    void getDesktopDatabase().then((db) => {
  ...
  async function loadReport(nextTab: ValuationTab = activeTab, cursor: string | null = null) {
    ...
    try {
      ...
      if (nextTab === "overview") {
        setOverview(await services.readAsOf({ companyId, branchId, asOfBusinessDate: jalaliToGregorian(asOfDate), productId: productId || null, warehouseId: warehouseId || null, limit: 100 }));
        ...
        setKardex(await services.readKardex({ companyId, branchId, productId, warehouseId, businessDateFrom: dateFrom ? jalaliToGregorian(dateFrom) : null, businessDateTo: dateTo ? jalaliToGregorian(dateTo) : null, cursor, limit: 100 }));
        ...
        const query = { companyId, branchId, productId: productId || null, warehouseId: warehouseId || null, fromBusinessDate: dateFrom ? jalaliToGregorian(dateFrom) : null, limit: 100 };
        const [unresolvedReport, candidates, nextStatus] = await Promise.all([
          services.readUnresolved(query),
          services.readInboundCostCandidates(query),
          services.readStatus(companyId, productId || null),
        ]);
  ...
  async function setInboundCost(row: InventoryInboundCostCandidate, unitCost: string) {
    ...
    setSuccess("");
    try {
      const result = await services.setManualInboundCost({ companyId: activeContext.companyId, movementId: row.movementId, unitCost, actorId: actor.id, requestId: crypto.randomUUID(), occurredAt: new Date().toISOString() });
      setSuccess(`بهای ورودی ثبت شد: ${new Intl.NumberFormat("fa-IR").format(result.totalCost)} ${result.currency}`);
      const query = { companyId: activeContext.companyId, branchId: activeContext.branchId || null, productId: productId || null, warehouseId: warehouseId || null, fromBusinessDate: dateFrom ? jalaliToGregorian(dateFrom) : null, limit: 100 };
      const [unresolvedReport, candidates, nextStatus] = await Promise.all([
        services.readUnresolved(query),
        services.readInboundCostCandidates(query),
  ...
  return (
    <Page className="valuation-page" dir="rtl">
      <header className="valuation-head">
        ...
        <div className={`valuation-status valuation-status--${valuationStatus}`}>{VALUATION_STATUS_LABELS[valuationStatus]}<small>Revision: <bdi>{status?.streamRevision ?? 0}</bdi></small></div>
      </header>
      ...
      <div className="valuation-tabs">{VALUATION_TABS.map(([tabId, label]) => <button key={tabId} className={activeTab === tabId ? "active" : ""} onClick={() => void loadReport(tabId)}>{label}</button>)}</div>
      <div className="valuation-filters">
        ...
        {activeTab === "overview" && <label>تا تاریخ<input dir="ltr" value={asOfDate} onChange={(e) => setAsOfDate(e.target.value)} /></label>}
        {(activeTab === "kardex" || activeTab === "unresolved") && <label>از تاریخ<input dir="ltr" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} /></label>}
        {activeTab === "kardex" && <label>تا تاریخ<input dir="ltr" value={dateTo} onChange={(e) => setDateTo(e.target.value)} /></label>}
      ...
      {activeTab === "unresolved" && <><ValuationInboundCostPanel rows={inboundCostCandidates} canResolve={services?.canResolveCostInput ?? false} saving={savingCost} onSetCost={setInboundCost} />{unresolved && <ValuationUnresolvedPanel report={unresolved} />}</>}{" "}
