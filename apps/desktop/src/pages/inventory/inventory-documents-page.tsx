...
import {
  ...
  type InventoryDocumentLineSnapshot,
...
const STATUS_LABELS: Record<InventoryDocumentStatus, string> = {
  draft: "پیش‌نویس",
  submitted: "ارسال‌شده",
  approved: "تأییدشده",
  confirmed: "قطعی",
...
function jalaliToGregorian(value: string): string { const normalized=latinDigits(value).trim().replace(/-/g,"/"); const match=/^(\d{4})\/(\d{1,2})\/(\d{1,2})$/.exec(normalized); if(!match) throw new Error("تاریخ را به‌صورت ۱۴۰۵/۰۶/۱۸ وارد کنید."); const jy=Number(match[1]),jm=Number(match[2]),jd=Number(match[3]); if(jm<1||jm>12||jd<1||jd>(jm<=6?31:jm<=11?30:30)) throw new Error("تاریخ شمسی معتبر نیست."); const g=d2g(j2d(jy,jm,jd)); return `${g.gy}-${String(g.gm).padStart(2,"0")}-${String(g.gd).padStart(2,"0")}`; }
...
function errorMessage(error: unknown): string {
  if (error instanceof InventoryApplicationError) {
    ...
    if (error.code === "inventory.application.invalid-request") return "اطلاعات سند یا وضعیت گردش آن برای این عملیات معتبر نیست.";
  }
...
export function InventoryDocumentsPage() {
  ...
  const [services,setServices] = useState<InventoryWorkspaceServices|null>(null);
  const [items,setItems] = useState<readonly import("@argin/inventory").InventoryDocumentListItem[]>([]);
  const [selected,setSelected] = useState<InventoryDocumentSnapshot|null>(null);
  ...
  const [newDraft,setNewDraft] = useState<NewDraft>({documentType:"receipt",businessDate:gregorianToJalali(new Date().toISOString().slice(0,10)),description:""});
  ...
  const [destinationZones,setDestinationZones] = useState<readonly WarehouseZoneDto[]>([]);
  const [destinationLocations,setDestinationLocations] = useState<readonly WarehouseLocationDto[]>([]);
  ...
  useEffect(() => { let mounted=true; void getDesktopDatabase().then((database)=>{ if(mounted)setServices(createInventoryWorkspaceServices({database,actor,audit})); }).catch((e)=>mounted&&setError(errorMessage(e))); return()=>{mounted=false;}; },[actor,audit]);
  ...
  const openDocument = useCallback(async(id:string)=>{ if(!services||!active.companyId)return; setError(""); try{const detail=await services.get(active.companyId,id);setSelected(detail?.document??null);}catch(e){setError(errorMessage(e));}},[services,active.companyId]);

  async function createDraft(event:FormEvent){event.preventDefault();if(!services||!active.companyId||!active.fiscalYearId)return;setSaving(true);setError("");try{const businessDate=jalaliToGregorian(newDraft.businessDate);const period=active.fiscalYears.find(y=>y.id===active.fiscalYearId);void period; const periods=await (await getDesktopDatabase()).query<{id:string}>("SELECT id FROM fiscal_periods WHERE fiscal_year_id=? AND start_date<=? AND end_date>=? AND status='open' ORDER BY start_date LIMIT 1",[active.fiscalYearId,businessDate,businessDate]); if(!periods[0])throw new Error("برای این تاریخ، دوره مالی باز پیدا نشد."); const doc=await services.createDraft({companyId:active.companyId,branchId:active.branchId||null,fiscalYearId:active.fiscalYearId,fiscalPeriodId:periods[0].id,documentType:newDraft.documentType,businessDate,description:newDraft.description.trim()||null});setNewOpen(false);setSelected(doc);setMessage("پیش‌نویس سند ایجاد شد.");await reload();}catch(e){setError(errorMessage(e));}finally{setSaving(false);}}

  async function startLine(line?:InventoryDocumentLineSnapshot){if(!services||!selected)return;setError("");try{const ps=await services.selectProducts(selected.companyId);const ws=await services.selectWarehouses(selected.companyId,selected.scope?.branchId??null);setProducts(ps);setWarehouses(ws);if(line?.operation){const p=await services.getProduct(selected.companyId,line.productId);setProduct(p);setLineDraft({lineId:line.lineId,productId:line.productId,quantity:line.operation.quantity.enteredQuantity,unitId:line.operation.quantity.enteredUnit.unitId,warehouseId:line.operation.warehouse.warehouseId,zoneId:line.operation.warehouse.zoneId??"",locationId:line.operation.warehouse.locationId??"",destinationWarehouseId:line.operation.destination?.warehouseId??"",destinationZoneId:line.operation.destination?.zoneId??"",destinationLocationId:line.operation.destination?.locationId??"",description:line.description??""});}else{setProduct(null);setLineDraft(emptyLine());}setLineOpen(true);}catch(e){setError(errorMessage(e));}}
  async function chooseProduct(productId:string){setLineDraft(d=>({...d,productId,unitId:""}));if(!services||!selected||!productId)return setProduct(null);const p=await services.getProduct(selected.companyId,productId);setProduct(p);if(p?.units)setLineDraft(d=>({...d,unitId:p.units?.baseUnitId??""}));}
  ...
  async function chooseZone(id:string,destination=false){if(!services||!selected)return;const warehouseId=destination?lineDraft.destinationWarehouseId:lineDraft.warehouseId;const ls=warehouseId&&id?await services.listLocations(selected.companyId,warehouseId,id):[];if(destination){setDestinationLocations(ls);setLineDraft(d=>({...d,destinationZoneId:id,destinationLocationId:""}));}else{setLocations(ls);setLineDraft(d=>({...d,zoneId:id,locationId:""}));}}

  async function saveLine(event:FormEvent){event.preventDefault();if(!services||!selected)return;setSaving(true);setError("");try{const operation=await services.buildOperation({companyId:selected.companyId,productId:lineDraft.productId,enteredQuantity:latinDigits(lineDraft.quantity),unitId:lineDraft.unitId,warehouseId:lineDraft.warehouseId,zoneId:lineDraft.zoneId||null,locationId:lineDraft.locationId||null,destinationWarehouseId:selected.documentType==="transfer"?lineDraft.destinationWarehouseId||null:null,destinationZoneId:lineDraft.destinationZoneId||null,destinationLocationId:lineDraft.destinationLocationId||null});const line=createInventoryDocumentLine({lineId:lineDraft.lineId??crypto.randomUUID(),position:lineDraft.lineId?(selected.lines.find(l=>l.lineId===lineDraft.lineId)?.position??selected.lines.length+1):selected.lines.length+1,productId:lineDraft.productId,description:lineDraft.description.trim()||null,operation});const lines=lineDraft.lineId?selected.lines.map(l=>l.lineId===line.lineId?line:l):[...selected.lines,line];const next=rehydrateInventoryDocument({...selected,lines,updatedAt:new Date().toISOString()});const saved=await services.saveDraft(next);setSelected(saved);setLineOpen(false);setMessage("ردیف سند ذخیره شد.");await reload();}catch(e){if(e instanceof InventoryApplicationError&&e.code==="inventory.application.concurrency-conflict")await openDocument(selected.documentId);setError(errorMessage(e));}finally{setSaving(false);}}
  async function removeLine(lineId:string){if(!services||!selected||!confirm("ردیف انتخاب‌شده حذف شود؟"))return;setSaving(true);try{const lines=selected.lines.filter(l=>l.lineId!==lineId).map((l,i)=>createInventoryDocumentLine({...l,position:i+1}));const saved=await services.saveDraft(rehydrateInventoryDocument({...selected,lines,updatedAt:new Date().toISOString()}));setSelected(saved);await reload();}catch(e){setError(errorMessage(e));}finally{setSaving(false);}}

  async function lifecycle(action:"submit"|"approve"|"confirm"|"cancel"|"reverse"){if(!services||!selected)return;const reason=action==="reverse"?prompt("علت برگشت سند را وارد کنید:",""):null;if(action==="reverse"&&!reason?.trim())return;setSaving(true);setError("");try{await services[action](selected,reason);await openDocument(selected.documentId);await reload();setMessage("عملیات با موفقیت انجام شد.");}catch(e){if(e instanceof InventoryApplicationError&&e.code==="inventory.application.concurrency-conflict")await openDocument(selected.documentId);setError(errorMessage(e));}finally{setSaving(false);}}

  const totalPages=Math.max(1,Math.ceil(total/50));
  const approvalRequestId=selected?`inventory-document:${selected.companyId}:${selected.documentId}`:"";
  return <Page title="اسناد انبار">
    <div className="inventory-workspace" dir="rtl">
      <div className="inventory-workspace__toolbar">
        <input aria-label="جست‌وجوی اسناد انبار" placeholder="جست‌وجو در شماره یا شرح سند…" value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} />
        <button type="button" onClick={()=>void reload()} disabled={loading}>بازخوانی</button>
        {can(inventoryPermissions.create)&&<button className="primary" type="button" onClick={()=>setNewOpen(true)}>سند جدید</button>}
      </div>
      {error&&<Feedback tone="danger">{error}</Feedback>}{message&&<Feedback tone="success">{message}</Feedback>}
      <div className="inventory-workspace__grid">
        <section className="inventory-list" aria-label="فهرست اسناد انبار">
          <div className="inventory-list__header"><span>تاریخ</span><span>نوع / شماره</span><span>وضعیت</span></div>
          {items.map(item=><button key={item.documentId} className={`inventory-list__row ${selected?.documentId===item.documentId?"is-active":""}`} onClick={()=>void openDocument(item.documentId)}>
            <span>{faDate.format(new Date(`${item.businessDate}T00:00:00Z`))}</span><span><strong>{TYPE_LABELS[item.documentType]}</strong><small dir="ltr">{item.documentNumber??"—"}</small></span><span className={`status status--${item.status}`}>{STATUS_LABELS[item.status]}</span>
          </button>)}
          {!loading&&items.length===0&&<p className="empty">سندی یافت نشد.</p>}
          <footer><button disabled={page<=1} onClick={()=>setPage(p=>p-1)}>قبلی</button><span>صفحه {page} از {totalPages}</span><button disabled={page>=totalPages} onClick={()=>setPage(p=>p+1)}>بعدی</button></footer>
        </section>
        <section className="inventory-detail" aria-label="جزئیات سند انبار">
          {!selected?<div className="empty">برای مشاهده جزئیات، یک سند را انتخاب کنید.</div>:<>
            <header><div><h2>{TYPE_LABELS[selected.documentType]}</h2><div className="meta"><span>شماره <b dir="ltr">{selected.documentNumber??"پیش‌نویس بدون شماره"}</b></span><span>تاریخ {gregorianToJalali(selected.businessDate)}</span><span>نسخه <b dir="ltr">{selected.version}</b></span></div></div><span className={`status status--${selected.status}`}>{STATUS_LABELS[selected.status]}</span></header>
            <div className="inventory-detail__actions">
              {selected.status==="draft"&&can(inventoryPermissions.edit)&&<button onClick={()=>void startLine()}>افزودن ردیف</button>}
              {selected.status==="draft"&&can(inventoryPermissions.submit)&&<button className="primary" disabled={saving||selected.lines.length===0} onClick={()=>void lifecycle("submit")}>ارسال برای تأیید</button>}
              {selected.status==="submitted"&&can(inventoryPermissions.approve)&&<button className="primary" onClick={()=>void lifecycle("approve")}>تأیید</button>}
              {selected.status==="approved"&&can(inventoryPermissions.confirm)&&<button className="primary" onClick={()=>void lifecycle("confirm")}>قطعی‌کردن و اثر موجودی</button>}
              {(["draft","submitted","approved"] as InventoryDocumentStatus[]).includes(selected.status)&&can(inventoryPermissions.cancel)&&<button className="danger" onClick={()=>void lifecycle("cancel")}>لغو</button>}
              {selected.status==="confirmed"&&can(inventoryPermissions.reverse)&&<button className="danger" onClick={()=>void lifecycle("reverse")}>برگشت کامل</button>}
              <button onClick={()=>navigate(`/approval/requests/${encodeURIComponent(approvalRequestId)}`)}>گردش تأیید</button>
            </div>
            <div className="inventory-summary"><div><span>شرح</span><strong>{selected.description??"—"}</strong></div><div><span>تعداد ردیف</span><strong dir="ltr">{selected.lines.length}</strong></div><div><span>آخرین تغییر</span><strong>{faDateTime.format(new Date(selected.updatedAt))}</strong></div></div>
            <div className="inventory-lines-wrap"><table className="inventory-lines"><thead><tr><th>#</th><th>کالا</th><th>مقدار</th><th>واحد</th><th>انبار / ناحیه / موقعیت</th>{selected.documentType==="transfer"&&<th>مقصد</th>}<th>شرح</th><th></th></tr></thead><tbody>{selected.lines.map(line=><tr key={line.lineId}><td dir="ltr">{line.position}</td><td dir="ltr">{line.productId}</td><td dir="ltr">{line.operation?.quantity.enteredQuantity??"—"}</td><td>{line.operation?.quantity.enteredUnit.title??"—"}</td><td dir="ltr">{line.operation?`${line.operation.warehouse.warehouseId}${line.operation.warehouse.zoneId?` / ${line.operation.warehouse.zoneId}`:""}${line.operation.warehouse.locationId?` / ${line.operation.warehouse.locationId}`:""}`:"—"}</td>{selected.documentType==="transfer"&&<td dir="ltr">{line.operation?.destination?.warehouseId??"—"}</td>}<td>{line.description??"—"}</td><td>{selected.status==="draft"&&can(inventoryPermissions.edit)&&<><button className="link" onClick={()=>void startLine(line)}>ویرایش</button><button className="link danger-text" onClick={()=>void removeLine(line.lineId)}>حذف</button></>}</td></tr>)}</tbody></table></div>
            <details className="inventory-history"><summary>تاریخچه گردش سند</summary>{selected.lifecycleHistory.length===0?<p>هنوز رویداد گردش ثبت نشده است.</p>:<ol>{selected.lifecycleHistory.map((h,i)=><li key={`${h.occurredAt}-${i}`}><b>{STATUS_LABELS[h.fromStatus]} ← {STATUS_LABELS[h.toStatus]}</b><span>{faDateTime.format(new Date(h.occurredAt))}</span><span>{h.reason??"بدون توضیح"}</span></li>)}</ol>}</details>
          </>}
        </section>
      </div>

      {newOpen&&<div className="inventory-modal" role="dialog" aria-modal="true" aria-label="ایجاد سند انبار"><form onSubmit={createDraft}><h2>سند انبار جدید</h2><label>نوع سند<select value={newDraft.documentType} onChange={e=>setNewDraft(d=>({...d,documentType:e.target.value as InventoryDocumentType}))}>{Object.entries(TYPE_LABELS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label>تاریخ شمسی<input dir="ltr" inputMode="numeric" placeholder="1405/06/18" value={newDraft.businessDate} onChange={e=>setNewDraft(d=>({...d,businessDate:e.target.value}))}/></label><label>شرح<textarea value={newDraft.description} onChange={e=>setNewDraft(d=>({...d,description:e.target.value}))}/></label><footer><button type="button" onClick={()=>setNewOpen(false)}>انصراف</button><button className="primary" disabled={saving}>ایجاد پیش‌نویس</button></footer></form></div>}
      {lineOpen&&selected&&<div className="inventory-modal" role="dialog" aria-modal="true" aria-label="ویرایش ردیف سند"><form className="line-form" onSubmit={saveLine}><h2>{lineDraft.lineId?"ویرایش ردیف":"ردیف جدید"}</h2><label>کالا<select required value={lineDraft.productId} onChange={e=>void chooseProduct(e.target.value)}><option value="">انتخاب کنید…</option>{products.map(p=><option key={p.productId} value={p.productId}>{p.code} — {p.title}</option>)}</select></label><label>مقدار<input required dir="ltr" inputMode="decimal" value={lineDraft.quantity} onChange={e=>setLineDraft(d=>({...d,quantity:e.target.value}))}/></label><label>واحد<select required value={lineDraft.unitId} onChange={e=>setLineDraft(d=>({...d,unitId:e.target.value}))}><option value="">انتخاب کنید…</option>{product?.units?.units.map(u=><option key={u.unitId} value={u.unitId}>{u.title} ({u.code})</option>)}</select></label><label>انبار<select required value={lineDraft.warehouseId} onChange={e=>void chooseWarehouse(e.target.value)}><option value="">انتخاب کنید…</option>{warehouses.map(w=><option key={w.warehouseId} value={w.warehouseId}>{w.code} — {w.title}</option>)}</select></label><label>ناحیه<select value={lineDraft.zoneId} onChange={e=>void chooseZone(e.target.value)}><option value="">بدون ناحیه</option>{zones.map(z=><option key={z.zoneId} value={z.zoneId}>{z.code} — {z.title}</option>)}</select></label><label>موقعیت<select value={lineDraft.locationId} onChange={e=>setLineDraft(d=>({...d,locationId:e.target.value}))}><option value="">بدون موقعیت</option>{locations.map(l=><option key={l.locationId} value={l.locationId}>{l.code} — {l.title}</option>)}</select></label>{selected.documentType==="transfer"&&<><label>انبار مقصد<select required value={lineDraft.destinationWarehouseId} onChange={e=>void chooseWarehouse(e.target.value,true)}><option value="">انتخاب کنید…</option>{warehouses.map(w=><option key={w.warehouseId} value={w.warehouseId}>{w.code} — {w.title}</option>)}</select></label><label>ناحیه مقصد<select value={lineDraft.destinationZoneId} onChange={e=>void chooseZone(e.target.value,true)}><option value="">بدون ناحیه</option>{destinationZones.map(z=><option key={z.zoneId} value={z.zoneId}>{z.code} — {z.title}</option>)}</select></label><label>موقعیت مقصد<select value={lineDraft.destinationLocationId} onChange={e=>setLineDraft(d=>({...d,destinationLocationId:e.target.value}))}><option value="">بدون موقعیت</option>{destinationLocations.map(l=><option key={l.locationId} value={l.locationId}>{l.code} — {l.title}</option>)}</select></label></>}<label className="wide">شرح ردیف<input value={lineDraft.description} onChange={e=>setLineDraft(d=>({...d,description:e.target.value}))}/></label><footer className="wide"><button type="button" onClick={()=>setLineOpen(false)}>انصراف</button><button className="primary" disabled={saving}>ذخیره ردیف</button></footer></form></div>}
    </div>
  </Page>;
}
