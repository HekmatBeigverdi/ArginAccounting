...
export function RoleManagement() {
  ...
  useEffect(
    () =>
      subscribeDesktopData(desktopDataTopics.securityRoles, () => {
        void loadRoles();
      }),
    [loadRoles],
  );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const database = await getDesktopDatabase();
      await createRole(new SqliteSecurityUnitOfWork(database), {
        code,
        title,
        description,
      });
    ...
    <div className="security-management-layout">
      <Panel className="security-feature-panel">
        <div className="security-feature-panel__heading">
          <div>
            <h3>نقش جدید</h3>
            <p>یک نقش سازمانی برای گروه‌بندی مجوزها تعریف کنید.</p>
          </div>
        </div>
        ...
        >
          <Field label="کد نقش" hint="برای مثال ACCOUNTANT">
            <Input
              value={code}
              dir="ltr"
          ...
          <div className="security-form__actions">
            <Button type="submit" variant="primary" disabled={isSubmitting}>
              {isSubmitting ? "در حال ذخیره..." : "ایجاد نقش"}
            </Button>
          </div>
        </form>
      </Panel>
      ...
      <Panel className="security-feature-panel">
        <div className="security-feature-panel__heading">
          <div>
            <h3>نقش‌ها</h3>
            <p>{roles.length.toLocaleString("fa-IR")} نقش تعریف‌شده</p>
          </div>
        </div>

        {loadError ? <Feedback tone="error">{loadError}</Feedback> : null}
        {isLoading ? (
          <p className="security-feature-muted">در حال دریافت نقش‌ها...</p>
        ) : null}
        {!isLoading && !loadError && roles.length === 0 ? (
          <Feedback tone="info">هنوز نقشی تعریف نشده است.</Feedback>
        ) : null}
        ...
        {roles.length > 0 ? (
          <DataTable>
            ...
            <tbody>
              {roles.map((role) => (
                <tr key={role.id}>
                  <td dir="ltr">{role.code}</td>
                  <td>{role.title}</td>
                  <td>
                    {role.isSystem ? (
                      <Badge tone="info">سیستمی</Badge>
                    ) : (
                      <Badge>عادی</Badge>
                    )}
                  </td>
