...
export function UserManagement() {
  ...
  useEffect(
    () =>
      subscribeDesktopData(desktopDataTopics.securityUsers, () => {
        void loadUsers();
      }),
    [loadUsers],
  );
    ...
    <div className="security-management-layout">
      <Panel className="security-feature-panel">
        <div className="security-feature-panel__heading">
          <div>
            <h3>کاربر جدید</h3>
            <p>حساب محلی جدید برای استفاده از نرم‌افزار بسازید.</p>
          </div>
        </div>
        ...
        >
          ...
          <label className="security-form__check">
            ...
            />
            <span>
              <strong>تغییر اجباری رمز در اولین ورود</strong>
              <small>
                کاربر پس از نخستین ورود باید رمز عبور خود را تغییر دهد.
              </small>
            </span>
          </label>
      ...
      <Panel className="security-feature-panel">
        <div className="security-feature-panel__heading">
          <div>
            <h3>کاربران</h3>
            <p>{users.length.toLocaleString("fa-IR")} حساب کاربری</p>
          </div>
        </div>

        {loadError ? <Feedback tone="error">{loadError}</Feedback> : null}
        {isLoading ? (
          <p className="security-feature-muted">در حال دریافت اطلاعات...</p>
        ) : null}
        {!isLoading && !loadError && users.length === 0 ? (
          <Feedback tone="info">هنوز کاربری ایجاد نشده است.</Feedback>
        ) : null}
              ...
              {users.map((user) => {
                ...
                return (
                  <tr key={user.id}>
                    ...
                    <td>{user.lastLoginAt ?? "بدون ورود"}</td>
                  </tr>
                );
              })}
            </tbody>
